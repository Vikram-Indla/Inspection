...
| `planning/planning-assistant` | server | 17 + 33 | The `/planning` AI band (T-022): a three-column grid (`1fr / 1.4fr / 0.9fr`) taking `insights`, `recommendations` and `quickActions` as slots. Collapses 3 → 2 (quick actions spanning) → 1. Pure layout — it knows nothing about AI. |
...
| `planning/planning-quick-actions` | server | 42 + 47 | Six real `<Link>` rows with `CountBadge` counts. Every href goes where its label promises: returned/draft/awaiting-supervisor filter the list, "Plan recommended" and "Build a bulk plan" enter `/planning/bulk`, "Create a visit" enters `/planning/single`. **"Generate Weekly Plan" links into the human bulk flow** rather than having AI create visits, which the AI docket forbids. |

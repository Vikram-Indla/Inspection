...
| `factories/factory-context` | server (used in client tree) | 93 + 41 | **Rebuilt by T-025.** Now composes the whole end panel: selected-context card, a `risk` slot (the reused `FactoryRisk` — score, band, model version, recorded driver breakdown), a latest-change card driven by the two most recent risk snapshots, an `advisory` slot, and a source card carrying provenance **plus two honest data-source states** (Senaei synced / risk engine calculated) rather than the mock's three unconditional ticks. Presentational — `risk` and `advisory` arrive as slots so the client boundary stays at the leaf. |

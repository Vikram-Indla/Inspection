...
| `planning-immediate/authority-bar` | **client** | 95 + 96 | **The nine `/planning/immediate` dispatch protections (T-051).** Supersedes the route-local `AuthorityBar`, whose nine chips carried their state as a `✓`/`✕`/`◌` glyph plus a colour class; each protection is now a `StatusPill` with a text state, so it survives greyscale (WEB-009 §12). **Zero hooks** — the component was 2 `useState`, 2 `useRef` and 2 `useEffect`, and every one of them was derived data wearing a disguise. The assertive announcement in particular needed no effect: it is a *derived string*, and React only mutates the text node when that string actually changes, which is exactly the "announce when the blocking set changes, not on every keystroke" behaviour the effect was hand-rolling. The view model lives in `features/planning-immediate/authority.ts`, so the component holds no policy. **A protection with no owning control is not a button.** Three of the nine (AUTHORIZED, AUDIT, NOTIFY) target nothing, and the legacy rendered them as focusable buttons that silently did nothing on Enter; they are now plain items, and the roving tabindex went with them — a `role="group"` of buttons is Tab-navigable by contract, roving tabindex belongs to `toolbar`/`listbox`. The remaining six focus their control by id: `getElementById` + `focus()` is a read plus focus management (WEB-012 §2), and the legacy `scrollIntoView({behavior:"smooth"})` is **deleted** — it ignored `prefers-reduced-motion`, and focusing already scrolls. **The blocking summary now lists every blocker**, in the same `role="alert"` region the visible echo occupies; the legacy showed the first blocker to sighted users and the full set to screen readers only, so a planner with four blocking protections was told about one. CD-023's accessible-name contract (`LABEL — state — detail`) is preserved verbatim, including uppercase English labels, which is why they are uppercase **in the resource** rather than via `text-transform` — Arabic has no case and `text.overline` would put tracking on it (WEB-011). |

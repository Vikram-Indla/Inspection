...
| `planning/planning-quick-actions` | server | 41 + 53 | **Not an AI panel** — no `accent="ai"`, and a `workflow` icon rather than the sparkle, because it is deterministic navigation with counted links and nothing about it is generated. Marking it as AI would defeat the accent's purpose. Six real `<Link>` rows with `CountBadge` counts. Every href goes where its label promises: returned/draft/awaiting-supervisor filter the list, "Plan recommended" and "Build a bulk plan" enter `/planning/bulk`, "Create a visit" enters `/planning/single`. **"Generate Weekly Plan" links into the human bulk flow** rather than having AI create visits, which the AI docket forbids. |

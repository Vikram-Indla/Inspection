...
| 6 | Focus to summary after submit; role=status progress (no optimistic success); native table (no ARIA grid) | **PASS** | Focus `VisitsBoard.tsx:315-317`; progress `role="status"` `509`; no `role="grid"` anywhere (grep clean) — plain `<table className="ax-table">`. |
...
| Original F3 — unchecked `expire_lapsed_visits` RPC (advisory) | **OPEN (advisory, non-blocking)** | `page.tsx:26/28` still awaits the RPC without inspecting its result. Best-effort remains acceptable for Track 1; noted for hardening, not a gate blocker. |
...
**Revised overall verdict: PASS.** All three remediation targets are genuinely closed in the real code, no other `{error.message}` (or equivalent error-object) JSX leak remains anywhere under `apps/web/src/app/visits/**`, the leak is now guarded by a code-layer regression test, and typecheck is clean. The single residual item (original F3, unchecked expiry RPC) is advisory hardening only and does not block CD-026 Track 1 sign-off.

...
| MAPBOX-EV-003 | Credentialed Inspection-staging Mapbox map/ETA/Arabic runtime journey | `TASK-IPAD-MAPBOX-RUNTIME-004.md` | Pending staging application configuration — Inspection DB `iiozvqntawxfwbgffzqu` read-only preflight passed; no staging app target or token variables are configured |
...
## TASK-REPO-DOC-EXTERNALIZE-001 — 2026-07-16

| ID | Item | File | Status |
|---|---|---|---|
| REPO-DOC-EV-001 | Complete discovery inventory and classification | `../../repo-cleanup/documentation_inventory.csv`; external `MANIFESTS/pre_move_inventory.csv` | Captured — 1,459 candidates; 1,004 D/E move; 455 A/B/C/F retain |
| REPO-DOC-EV-002 | Verified pre-move backup | external `10_REPOSITORY_EXPORTS/Inspection_Documentation_PreMove_Backup_20260716T190000+0300.zip` | PASS — 1,459 entries; archive test PASS; SHA-256 recorded externally |
| REPO-DOC-EV-003 | Per-file copy/removal proof | external `MANIFESTS/move_plan.csv`; `files_removed_from_repo.csv` | PASS — 1,004/1,004 destination hashes and sizes match |
| REPO-DOC-EV-004 | Reference and symlink integrity | external `MANIFESTS/documentation_link_check.txt`; `broken_references_fixed.csv` | PASS — 221 external references; 0 broken; one intentional runtime-dependency symlink resolves |
| REPO-DOC-EV-005 | Validation and rollback record | external `MANIFESTS/validation_results.md`; `rollback_instructions.md` | Captured — push withheld on recorded pre-existing YAML/i18n exceptions; live-data E2E safety exclusion explicit |
...
| SHELL-EV-005 | V3 43-screen approval pack verification | `${INSPECTION_DOCS_ROOT}/05_UI_UX_AND_STORYBOARDS/outputs/claude-design-approval-pack/VERIFICATION_REPORT_V3.json` | Captured — 11 sheets; 43 prompts; 22 tabs; 0 formula errors |
...
| ID | Item | File | Status |
|---|---|---|---|
| CD025-R3-EV-001 | P0/P1 review of `Plan Review and Publish (1).zip` | `${INSPECTION_DOCS_ROOT}/05_UI_UX_AND_STORYBOARDS/outputs/claude-design-approval-pack/CD-025_DESIGN_REVIEW_R3.md` | Captured — P0 none; four P1; archive BLOCKED |
| CD025-R4-EV-001 | Focused package-synchronization correction prompt | `${INSPECTION_DOCS_ROOT}/05_UI_UX_AND_STORYBOARDS/outputs/claude-design-approval-pack/CD-025_PROGRESSIVE_CORRECTION_PROMPT_R3.md` | Prepared — awaiting Claude Design R4 resubmission |
...
| CD026-EV-001 | Complete fresh-session Claude Design prompt | `${INSPECTION_DOCS_ROOT}/05_UI_UX_AND_STORYBOARDS/outputs/claude-design-approval-pack/CD-026_CLAUDE_DESIGN_END_TO_END_R1_FRESH_SESSION_PROMPT.md` | Prepared — design not yet generated or approved |
...
| ID | Item | File | Status |
|---|---|---|---|
| CD027-EV-001 | Recorded 14-leg wiring audit + Track 2 closure log (DEC-012) | `${INSPECTION_DOCS_ROOT}/05_UI_UX_AND_STORYBOARDS/outputs/claude-design-approval-pack/CD-027_WIRING_AUDIT_R1.md` | Captured — all 14 legs wired; ERRORMAP/ORPHAN/NOTIFY_PREV closed; MAP/ASSIGNMENT_RELEASE/ATOMIC held blocked |

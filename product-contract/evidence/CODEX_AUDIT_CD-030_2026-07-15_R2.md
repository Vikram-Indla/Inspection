...
- `${INSPECTION_DOCS_ROOT}/05_UI_UX_AND_STORYBOARDS/outputs/claude-design-approval-pack/CD-030_WIRING_AUDIT_R1.md:3`: the implementer's own scope line reads **"Scope: `/reviews/:id` route-neutral compare mode · P11 · Reviewer/Auditor."** — Auditor is explicitly in this screen's intended audience, not an edge case.
- `${INSPECTION_DOCS_ROOT}/05_UI_UX_AND_STORYBOARDS/outputs/claude-design-approval-pack/CD-030_PROGRESSIVE_CORRECTION_PROMPT_R1.md:40` explicitly lists `"auditor read-only"` as one of the required state-matrix rows that must be proven with a populated screenshot — i.e., "auditor sees a read-only version of this screen" was already a known, named requirement before this session's change.

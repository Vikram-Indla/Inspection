...
- **Race-guard migration CONFIRMED LIVE (2026-07-15, later same day):** first application attempt failed (`23505 could not create unique index — Key (submission_version_id)=(91116402-...) is duplicated`) — two pre-existing `under_review`/undecided rows for the same submission version already existed, almost certainly artifacts of repeated `golden-journey` P3 runs against the same seeded inspection before this constraint existed (`CREATE UNIQUE INDEX` refuses to apply over pre-existing violations). Sponsor deleted the duplicates via SQL Editor (`reviews_delete` has no RLS policy, so this is only possible with SQL Editor's elevated access, not the anon/ops REST path) and re-ran the migration successfully. Verified independently via a direct REST insert attempt against the surviving row's `submission_version_id`: **rejected with `409 / 23505 duplicate key value violates unique constraint "reviews_one_open_per_version"`.** The discoverability fix (RLS + Option B query + this race guard) is now fully live and functionally confirmed end-to-end, not just drafted.
- Reopen rule: demonstrated P0/P1 regression, a11y/security failure, protected-behavior break, or recorded release blocker only.

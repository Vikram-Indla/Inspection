...
- **DSG-A11Y-001** (semantic, RTL, responsive, non-color): **IMPLEMENTED AND VERIFIED** — ribbon as keyboard `tablist` (APG roving tabindex, Arrow/Home/End); glyph+label status (FND-011); `role=status` completion / single `role=alert` failure; 412 narrow reflow to ordered ledger. RTL/theme parity carried by unchanged tokens/retired-predecessor.

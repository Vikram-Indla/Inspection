...
1 Tokens → 2 Fonts (purge Barlow/retired predecessor) → 3 Shared primitives (Button, Field/Input, StatusBadge, panel) → 4 App shell (Sidebar/TopBar/layout) → 5 Navigation (PageHeader/Breadcrumb/Tabs/Steps) → 6 Forms → 7 DataGrid (register first) → 8 Map controls (engine untouched) → 9 Inspection + signature components → 10 Representative pages → 11 Remaining pages → 12 retired predecessor removal → 13 Final verification (VISUAL_QA_MATRIX × four modes).
...
Renders in all four modes · no retired predecessor imports · no raw hex/font declarations · focus ring present · status vocabulary canonical · passes the view's row in RETIRED PREDECESSOR_MIGRATION_TEMPLATE.md.

...
You are implementing the SAQEEL Inspection Design System across this repository, replacing Astryx entirely. The design system is the sole visual authority — invent nothing.
...
- Migration map with owner-resolved decisions: design/saqeel/handoff/ASTRYX_MIGRATION_TEMPLATE.md. The new spec supersedes ALL Astryx decisions (16px body minimum, 44–52px blanket controls, Space Grotesk input freeze, JetBrains Mono, steel-blue dark primary — all retired; log supersessions in the product decision register).

SEQUENCE (design/saqeel/handoff/IMPLEMENTATION_SEQUENCE.md)
Start with step 1 ONLY, as one PR:
1. Replace apps/web/src/app/tokens.css content with the SAQEEL token sheet (map --ax-* consumers via the token table in the migration map; keep the data-theme mechanism and ThemeScript).
2. Swap fonts: IBM Plex Sans / IBM Plex Sans Arabic / IBM Plex Mono; remove Space Grotesk + JetBrains Mono loads.
...
Renders in all four modes; no Astryx imports; no raw hex/font declarations; focus ring on every interactive element; status vocabulary = the 10 canonical roles; touch targets ≥44px on field surfaces.

FINAL GATE (step 12–13)
Delete apps/web/src/app/astryx.css, ax tokens, design/astryx/, outputs/cd-* styling packs, MIM_Inspection_Meta_Astryx_Fable_Pack. Then repo-wide grep `ax-|astryx|Space Grotesk|JetBrains|Barlow` must return zero product-code hits, and the QA matrices (VISUAL / RESPONSIVE / ACCESSIBILITY) must pass.

FIRST ACTION
Inventory every `--ax-*` consumer and `.ax-*` usage (file + count), confirm it maps to a row in the migration map, and report any unmapped usage BEFORE writing code. If any visual decision is missing from the design system, stop and escalate to Claude Design — do not invent.

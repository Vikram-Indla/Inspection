...
5. `design/saqeel/handoff/RETIRED PREDECESSOR_MIGRATION_TEMPLATE.md` — the populated retired predecessor→SAQEEL migration map with owner-resolved decisions.
...
- SAQEEL control metrics (28/32/40 comfortable, 24/28/36 compact, 40/44/52 field density) **supersede** the blanket 44–52px retired predecessor heights. Field/iPad surfaces get `data-density="field"`.
- The frozen input contract (retired input font, 12px input radius) is **retired**: inputs are IBM Plex Sans at 3px radius.
- retired mono font is **retired**: IBM Plex Mono, identifiers only.
...
Before writing any code, produce `design/saqeel/handoff/RETIRED PREDECESSOR_INVENTORY_REPORT.md`:
1. `grep -rn "var(--legacy-" apps/ | …` — every `--legacy-*` token consumer: file, line count per token.
2. `grep -rn "legacy-" apps/web/src --include="*.tsx" --include="*.ts" --include="*.css"` — every `.legacy-*` class usage by file.
...
4. Font loads: retired input font, retired mono font, IBM Plex Sans Arabic (currently applied to English too), Barlow (if any).
5. The four design-contract Playwright specs and every assertion they make against retired-predecessor.css/tokens.css.
6. Cross-check EVERY item against the mapping table in `RETIRED PREDECESSOR_MIGRATION_TEMPLATE.md`. Output three lists: **mapped** (with target SAQEEL component), **out-of-scope** (login atlas, historical packs), **UNMAPPED** — anything unmapped goes in the report and to "ESCALATE TO CLAUDE DESIGN"; do not improvise a mapping.
Commit this report alone as the first commit.

# 3. IMPLEMENTATION SEQUENCE — one PR per step, in order

Never mix retired predecessor and SAQEEL styling within one view beyond a single release window. Each PR: implementation + tests + the verification checklist from §8 for the surfaces it touches.
...
b. Add a compatibility shim ONLY if consumer count makes a single-PR rename impractical: `--legacy-color-primary: var(--action-primary);` etc. per the token table in the migration map. If you shim, file the shim's removal as part of PR 12 and track every shimmed name.
c. Fonts: load IBM Plex Sans (400/500/600/700), IBM Plex Sans Arabic (400/500/600/700), IBM Plex Mono (400/500) — self-hosted with `font-display: swap` preferred, Google Fonts acceptable interim. Remove retired input font and retired mono font loads. English text must resolve to IBM Plex Sans (not Plex Arabic); Arabic (`:lang(ar)` / `[dir=rtl]`) resolves Arabic-first with raised leading per `foundations/TYPOGRAPHY_SPEC.md`.
...
Map every `.legacy-btn*`, `.legacy-field/.legacy-input/.legacy-select/.legacy-textarea/.legacy-search`, `.legacy-choice/.legacy-check/.legacy-switch/.legacy-segmented`, `.legacy-lozenge/.legacy-badge` consumer to these per the migration map. Delete the replaced `.legacy-*` rules from retired-predecessor.css as consumers migrate (shrinking file = progress metric).
...
## PR 12 — retired predecessor removal
Delete: `apps/web/src/app/retired-predecessor.css`, all `--legacy-*` definitions (and any PR-1 shim), `design/retired-predecessor/`, `saqeel-retired-predecessor.css` historical packs, `outputs/cd-*` styling packs, `MIM_Inspection_Meta_retired predecessor_Fable_Pack`, retired input font/retired-mono font files+loads, dead exports. Zero-trace grep gate (§9) must pass in CI.
...
[ ] No `.legacy-*` / `--legacy-*` additions; net `.legacy-*` usage decreased (count before/after)
...
`grep -rEni "legacy-|retired-predecessor|retired input font|retired mono font|barlow|sakil" apps/ packages/ --include="*.{ts,tsx,css,scss,json,html}"` returns ZERO hits in product code (allowed: the decision register, migration docs, git history). Fonts directory contains only IBM Plex families. Bundle analysis shows no retired predecessor CSS. All four contract specs + full E2E suite green. Then and only then squash-merge the removal PR.

# 10. REPORTING
After PR 1 and after every subsequent PR, append to `design/saqeel/handoff/IMPLEMENTATION_LOG.md`: date, PR, step, surfaces migrated, `.legacy-*` count remaining, escalations raised, decisions logged. If at ANY point a needed visual decision is not in the design package — a component, a state, a token, an Arabic treatment — do not invent it: record it under ESCALATE TO CLAUDE DESIGN and continue with mapped work.

...
| Binding exclusions | **Mobbin is excluded** (no Mobbin MCP, no Mobbin reference used). **Historical archives are provenance only.** **Meta-retired predecessor pack is design direction only, not approved design authority (G6 not passed).** **Unresolved decisions DEC-001..010 / OD-01..08 may not be invented or resolved by the agent.** |
| Design status | No retired predecessor artifact, token page, component or product screen has been created. |
...
**Date:** 2026-07-11
**Authority order applied:** (1) reconciled repository `product-contract/` + `docs/` + `CLAUDE.md`; (2) historical archives in `MIM_Inspection_MVP1_Historical_Archives_v3/` for completeness/provenance only; (3) `MIM_Inspection_Meta_retired predecessor_Fable_Pack/` for design direction only.
...
- **Mandatory screen states (all channels):** loading, empty, populated, validation failure, unauthorized, read-only/immutable, stale data, degraded service, offline, syncing, conflict, success confirmation *(Meta-retired predecessor pack `00_DESIGN_SYSTEM_DECISION.md` "Non-negotiable states"; consistent with per-screen `states` column in the screen catalogue and `.claude/rules/web.md`)*.
- **Design direction (authority level 3, pre-G6):** MIM retired predecessor — Meta-inspired enterprise system; Inter / IBM Plex Sans Arabic; primary retired predecessor Cobalt `#0866FF` (token source); 4px spacing base; radii 8/12/16; desktop controls 40–44px; iPad targets ≥48px; tabular numerals for KPIs/coordinates/timers; component library (core + enterprise incl. workflow canvas, rule builder, evidence gallery, conflict resolver, offline/sync indicator, immutable banner) built before screens; six golden screens then end-to-end prototype; three-approval sequence *(Meta-retired predecessor pack: `00_DESIGN_SYSTEM_DECISION.md`, `01_FABLE_MASTER_EXECUTION_PROMPT.md` §3–6, `02_APPROVAL_SEQUENCE.md`)*. Figma tokens/golden screens become authoritative only after G6 approval *(repo `CLAUDE.md` Design policy; `product-contract/design/DESIGN_AUTHORITY_STATUS.md`)*.
...
- **Arabic/RTL:** MVP1 scope (English-only vs bilingual vs selected surfaces) is **undecided — DEC-004/OD-01**, required by G6 design freeze; do not assume. Design direction prepares for it: IBM Plex Sans Arabic, native RTL via logical layout and mirrored directional behavior, RTL variants for representative complex screens *(Meta-retired predecessor pack)*. Full bilingual authoring/translation tooling = MVP2-016.
- **Responsive:** web/admin legible at standard enterprise resolutions; desktop, laptop, tablet layouts; iPad portrait+landscape deliberate; "desktop pages compressed into an iPad viewport" is a design exclusion *(MVP1-FND-011; Meta-retired predecessor pack; iPad spec §8)*.
...
1. **Missing `Requirements` folder:** Meta-retired predecessor pack (`00_DESIGN_SYSTEM_DECISION.md` "Authority path"; `01_FABLE_MASTER_EXECUTION_PROMPT.md`) instructs reading `/Users/vikramindla/Documents/GitHub/Inspection/Requirements` — that folder does not exist. Actual requirement authority lives in `product-contract/` and the historical archive. Needs correction or confirmation.
2. **Mobbin conflict:** repo `CLAUDE.md` design policy, `GATE_STATUS.md` G6 title, archive `MIM_Inspection_MVP1_UIUX_Design_Authority_Mobbin_MCP.pdf` and Meta-retired predecessor execution prompt §2 all mandate Mobbin MCP pattern research; the current instruction explicitly excludes Mobbin. Treated here as the latest human instruction overriding earlier direction; the written contract should be amended via change control before G6.
3. **Two design languages:** archive UI/UX authority prescribes "deep navy" restrained enterprise direction; Meta-retired predecessor pack (newer, 2026-07-11) prescribes Meta-inspired MIM retired predecessor with Cobalt #0866FF. retired predecessor is authority level 3 (design direction) and supersedes the archive doc per the pack's own decision statement, but **neither is G6-approved** — no design authority is frozen yet (`design/DESIGN_AUTHORITY_STATUS.md`).
...
7. **retired predecessor pre-empts DEC-004:** the pack mandates Arabic typography/RTL rules while Arabic scope (DEC-004) is open. Prepare-for-RTL is safe; committing bilingual scope is not.
...
*Meta-retired predecessor pack (authority 3):* `00_DESIGN_SYSTEM_DECISION.md`, `01_FABLE_MASTER_EXECUTION_PROMPT.md`, `02_APPROVAL_SEQUENCE.md`, `PACK_MANIFEST.json`.
...
4. Correct/confirm the Meta-retired predecessor pack's `Requirements` folder path.
...
6. Confirm MIM retired predecessor as the G6 design authority via Approval 1 (foundation) before any screen work.
...
- Design authority: **Medium** — retired predecessor direction fully absorbed, but G6 NOT STARTED and no design artifact is approved; Mobbin exclusion noted.

...
## Responsive — read this before building layout

**The file has one width: 1280px.** The responsive contract is not in Figma, it is in the
CSS, and the CSS is authoritative. Do not infer breakpoint behaviour from the frames.
...
1. **AR frames are detached.** 983 instances against 2,265 in EN. Figma has no RTL
   ...
   component edit will not reach the AR sections; they must be rebuilt from EN when a
   component changes. This is a Figma limitation, not a choice.
2. **Arabic is partial** — 59% on the newer screens. 262 strings have no approved
   translation and render English. See the translation packet.

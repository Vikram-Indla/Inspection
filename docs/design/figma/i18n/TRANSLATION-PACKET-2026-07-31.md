...
- **8 rows** are near matches the build rejected and the Product Owner applied anyway (ruling 2026-07-31). They render in Figma, but each carries a stated reservation in `owner-accepted-suggestions.json` and stays in the outstanding count — re-check them first.
- **178 rows** are genuinely new copy.

So the real ask is **213 rows**, not 311.
...
1. Fill the `ar` column in `TRANSLATION-PACKET-2026-07-31.csv` for every row where `status` is `new` or `suggest` — they sort to the top of each priority band. Leave a row blank if the English is wrong; that is a copy defect, not a translation task.
...
Counts above are all rows. Subtract `reuse` and `accepted` for what is actually outstanding: P1 111, P2 38, P3 64.
...
| `planning.bulk.selection` | Selection | SCR-WEB-110 | section-title | Section |
| `planning.configure.conflictPanel` | Conflict panel | SCR-WEB-140 | section-title | Conflict · {n} |
| `planning.immediate.assignment` | Assignment | SCR-WEB-130 SCR-WEB-210 | section-title | Assignments |
| `planning.immediate.reasonForImmediateVisit` | Reason for immediate visit | SCR-WEB-130 | form-label | Reason for immediate inspection |
| `planning.review.validation` | Validation | SCR-WEB-150 | section-title | Validator |
| `reviews.compare.comparedVersions` | Compared versions | SCR-WEB-320 | section-title | Compare versions |
| `reviews.compare.inspectionWindow` | Inspection window | SCR-WEB-320 | field-term | Shared inspection window |
| `reviews.workspace.findings` | Findings | SCR-WEB-310 | section-title | Finding |
| `reviews.workspace.queue` | Queue | SCR-WEB-310 | section-title | Queued |
| `reviews.workspace.unverified` | Unverified | SCR-WEB-310 | form-label | Verified |
| `virtual.appointment.deviceReadiness` | Device readiness | SCR-VIR-700 | section-title | Review readiness |
| `virtual.session.sessionState` | Session state | SCR-VIR-720 | field-term | Session state path |
| `virtual.verify.identityVerificationVs40066` | Identity verification · VS-40066 | SCR-VIR-710 | page-title | Identity Verification |
| `virtual.verify.otpState` | OTP state | SCR-VIR-710 | section-title | To state |
| `virtual.verify.sessionLocked` | Session locked | SCR-VIR-710 | alert-title | Session closed |
| `visits.detail.schedule` | Schedule | SCR-WEB-210 | section-title | Scheduled |
| `visits.list.planningStatusAndOperationalState` | Planning status and operational state are two independent tracks and are never merged. Rescheduling, reassignment, cancellation, bulk actions and publish/return are not available here. | SCR-WEB-200 | alert-body | Read-only workspace. Planning status and operational state are two independent tracks and are never merged. Rescheduling, reassignment, cancellation, bulk actions and publish/return are not part of this slice (CR-071). |

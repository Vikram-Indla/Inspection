...
- **Test execution (IMPORTANT — overrides any `swift test` shown in task steps):** the Core package imports `UIKit`/`UIColor`, which are unavailable on the macOS host that `swift test` uses. Therefore run ALL tests on the iPad simulator via xcodebuild.
  - Core package (Tasks 1–7): from `apps/ipad/Core`, run
    `xcodebuild test -scheme InspectionCore-Package -destination 'platform=iOS Simulator,name=iPad Pro 11-inch (M4)' -quiet`
    (the aggregate scheme is auto-generated by SwiftPM; if not found, run `xcodebuild -list` from `apps/ipad/Core` to discover the exact scheme name). Run the whole suite — do not rely on `--filter`.
  - App target (Tasks 9–11): from `apps/ipad`, run
    `xcodebuild test -project InspectionApp.xcodeproj -scheme InspectionApp -destination 'platform=iOS Simulator,name=iPad Pro 11-inch (M4)' CODE_SIGNING_ALLOWED=NO`
  - The "verify it fails" step still applies: run the command, confirm a compile/test failure for the new symbol before implementing.
- Toolchain present: Xcode 26.1.1, Swift 6.2 toolchain, xcodegen 2.45.4. `SWIFT_VERSION: "5.9"` in build settings pins the language mode, so avoid Swift 6 strict-concurrency-only constructs.

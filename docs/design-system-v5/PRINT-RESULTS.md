...
- **many-violations, long-notes**: **not synthesized, deliberately.** Producing them would require inserting a new row directly into `submission_versions` — an explicitly immutable, workflow-governed table. `scripts/seed-dashboard-kpis.mjs` (this repo's own precedent for RLS-respecting fixture seeding) itself stops at `inspections.status` and never fabricates a submission version, for the same reason: CLAUDE.md's "Never mutate workflow status directly; use canonical transitions and guards" and "Never edit an immutable submitted version" apply to *creating* one via a raw insert that bypasses the real submit workflow, not just editing an existing one. This is a governance boundary, not an effort shortfall.
...
- many-violations / long-notes fixtures and 20/100/300-item package fixtures remain unexercised, for the governance reasons above — closing them requires either a change-control-approved workflow-level fixture-seeding capability (a submit-flow RPC/helper, not a raw table insert) or an approved new checklist-package catalogue entry, both outside this CC's scope.

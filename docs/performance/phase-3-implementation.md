...
| K-013/K-014 DB migration | `57f7b1e` | `supabase/migrations/20260721090000_perf_indexes_rls_initplan.sql` — 7 indexes on hot columns + `(select auth.uid())` initplan wrap in `has_role`/`has_any_role`/`is_assigned_inspector`. **NOT applied to any live DB — needs review/apply.** |

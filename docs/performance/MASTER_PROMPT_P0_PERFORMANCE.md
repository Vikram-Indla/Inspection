...
Because three different CLIs (Codex, Kimi K3, Claude Code) touch this branch across passes, **each CLI works in its own git worktree pointed at the same branch** — this avoids one CLI's uncommitted state colliding with another's, while all commits land on one shared branch/history:

```bash
git worktree add ../Inspection-perf-codex  perf/p0-navigation-remediation   # Pass 1, Pass 4 — Codex
git worktree add ../Inspection-perf-kimi   perf/p0-navigation-remediation   # Pass 2 — Kimi K3
git worktree add ../Inspection-perf-claude perf/p0-navigation-remediation   # Pass 3 — Claude Code
```

Rules:
* Never force-push.
* Never use destructive reset against uncommitted work.
...
* Work sequentially per pass (Pass 1 → 2 → 3 → 4) — do not run two agents writing to `perf/p0-navigation-remediation` at the same instant, even from different worktrees.
* Before starting each pass: `git fetch origin && git rebase origin/perf/p0-navigation-remediation` (or pull --rebase) in that agent's worktree.
* Before every handover: push validated commits to `origin/perf/p0-navigation-remediation`.

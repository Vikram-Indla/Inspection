...
| **B4 ~70% (2026-07-11)** | LIVE: /planning/single working wizard (CR-only search, risk badge, published-package select, inspector assignment, M01-041 validation gate with exact blockers incl. duplicate detection M02-012, atomic publish + notification, redirect to dossier) · /visits/[id] dossier (config, assignment, versions, reviews, immutable geo timeline with gis version stamps) · /factories + /factories/[id] Factory 360 (identity read-only, reproducible risk, full inspection history with violations/actions/outcomes). Migrations 0007/0008 (role visibility). Remaining: bulk + immediate flows, visit edit/reassign/cancel actions | M01 single-path + M07 core ACs surfaced |

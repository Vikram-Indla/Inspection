-- Read-only Admin planning capability proof. The request claim is
-- transaction-local; no data, account, session, or role assignment is modified.
with actor as (
  select user_id::text as id from public.user_roles where role_key = 'admin' order by user_id limit 1
), claim as (

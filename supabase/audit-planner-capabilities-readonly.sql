-- Read-only Planner capability proof. The request claim exists only for this
-- query transaction; no data, account, session, or role assignment is changed.
with actor as (
  select user_id::text as id from public.user_roles where role_key = 'planner' order by user_id limit 1
), claim as (

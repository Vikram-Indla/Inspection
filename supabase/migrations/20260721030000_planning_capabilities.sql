...
-- Legacy business roles map to the full business planning set. planner/ops
-- additionally keep the immediate-visit creation they already have today
-- (create_immediate_visit planner mode), which the business-staff class
-- default deliberately does NOT grant.
...
-- Inspector: the Immediate-Visit exception only (PLANNING_ROLE_ACCESS matrix).
insert into public.role_permissions (role_key, permission_key)
select r.role_key, 'planning.create.immediate'
  from public.roles r
...
-- Admin class: configure capabilities only — no business data access.
insert into public.role_permissions (role_key, permission_key)
select r.role_key, p.permission_key
  from public.roles r
  join (values
...
-- admin.access.manage is a security_admin capability inside the Admin class.
insert into public.role_permissions (role_key, permission_key)
...
-- Canonical classes per PLANNING_ROLE_ACCESS_ADMIN_MODEL:
--   admin          — any is_admin role (admin takes precedence over inspector)
--   inspector      — explicit inspector role, not admin
--   business_staff — DEFAULT: any other authenticated internal user
...
-- Explicit grants (permissions/role_permissions via has_permission) always
-- win. Class defaults apply ONLY to the listed business capabilities;
-- high-impact capabilities and admin.access.manage require an explicit grant.
...
as $$
  select case
    ...
    -- High-impact / access-management capabilities: explicit grant only.

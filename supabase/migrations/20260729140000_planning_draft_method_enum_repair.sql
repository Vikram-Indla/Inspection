-- REQ-013 — forward-only repair for governed Planning draft creation.
-- Preserve the existing RPC contract, evidence, grants and records while
-- making the validated text method explicit at the planning_method boundary.
...
comment on function public.save_planning_draft_atomic(
  uuid,text,jsonb,jsonb,integer,text,uuid
) is
  'REQ-013 governed idempotent Planning draft save; explicit planning_method enum boundary.';

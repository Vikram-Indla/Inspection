-- Fix duplicate_terminal_visit_atomic: v_method (declared text) was inserted
-- directly into visit_plans.method (planning_method enum) with no cast.
--
-- Defect (found by cd-027-visit-detail.spec.ts "duplicate clones a cancelled
-- visit into a safe new draft", PLN-REQ-011): every call fails with SQL
-- 42804 — column "method" is of type public.planning_method but expression
-- is of type text. Duplicate Visit has been non-functional for every caller
-- since 20260728010000_planning_closure_p0.sql landed.
--
-- Verbatim copy of the function as landed in that migration (lines
-- 1522-1653), with the single insert on line 1609 cast to the enum. No other
-- line changed.

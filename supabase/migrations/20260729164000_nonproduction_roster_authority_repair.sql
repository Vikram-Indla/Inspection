-- Keep non-production roster recommendations on the same explicit capability
-- boundary as the final Planning closure policies. The prior synthetic-roster
-- migration used the retired has_capability helper, which left a Planner able
-- to see the roster but unable to validate it at schedule time.

-- Analytics canonical-role convergence.
--
-- Replaces public.analytics_metric_snapshot (unchanged body from
-- 20260727120000_analytics_metrics_expansion.sql) with its role checks widened
-- onto the four canonical internal roles introduced by
-- 20260728012000_four_canonical_roles_compatibility.sql. That migration did
-- not touch this function's authorization ("existing RLS remains unchanged
-- until its policy family is migrated and tested"), so every governed UAT
-- identity (docs/TEST_ACCOUNTS.md) and every canonical-role account carries
-- role_key in ('admin','planner','inspector','supervisor') only — none of
-- which satisfied the legacy literals this function checked
-- ('leadership','ops','reviewer','auditor'). ANALYTICS_ROLE_REQUIRED fired for
-- every admin/supervisor caller and Analytics could not load.
--
-- Widening uses the equivalence public.role_canonical_mappings already
-- records (in the reverse direction): ops/reviewer/auditor/leadership ->
-- supervisor, security_admin/compliance_admin -> admin. Scope stays as
-- documented in docs/TEST_ACCOUNTS.md (Admin = national, Supervisor =
-- regional): 'admin' joins the national-scope leadership/auditor check,
-- 'supervisor' does not. Every legacy literal stays in place — no existing
-- holder loses access. security invoker is unchanged; underlying table RLS
...
begin
  ...
  if not public.has_any_role(array['leadership','ops','planner','reviewer','auditor','admin','supervisor']) then
  ...
  v_is_national := public.has_any_role(array['leadership','auditor','admin']);
  v_can_read_compliance := public.has_any_role(array['leadership','ops','reviewer','auditor','admin','supervisor']);
  v_can_read_assignments := public.has_any_role(array['planner','ops','reviewer','auditor','admin','supervisor']);
  v_can_read_geo := public.has_any_role(array['ops','reviewer','auditor','admin','supervisor']);
  v_can_read_reviews := public.has_any_role(array['leadership','ops','reviewer','auditor','admin','supervisor']);
  ...
  v_can_read_evidence := public.has_any_role(array['planner','ops','reviewer','auditor','admin','supervisor']);
  -- Mirrors notifications RLS (notif_own): only ops reads beyond own recipient row.
  -- supervisor/admin are the canonical successors of ops for this read.
  v_can_read_notifications := public.has_any_role(array['ops','admin','supervisor']);
...
is 'AN-AC-004..018 proposed child traces. SECURITY INVOKER aggregate read; underlying RLS remains authoritative. Twenty-six governed metric rows. Converged onto canonical roles 2026-08-03 (admin/supervisor added per role_canonical_mappings equivalence).';

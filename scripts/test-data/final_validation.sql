...
select 'auth_users' entity,count(*) from auth.users where email ~ '^(admin|planner|supervisor|inspector)[0-9]+@mim\.gov\.sa$'
union all select 'profiles',count(*) from public.profiles where email ~ '^(admin|planner|supervisor|inspector)[0-9]+@mim\.gov\.sa$'
union all select 'user_roles',count(*) from public.user_roles ur join public.profiles p on p.user_id=ur.user_id where p.email ~ '^(admin|planner|supervisor|inspector)[0-9]+@mim\.gov\.sa$'
...
union all select 'notifications',count(*) from public.notifications where recipient in (select user_id from public.profiles where email ~ '^(admin|planner|supervisor|inspector)[0-9]+@mim\.gov\.sa$')
union all select 'audit_events',count(*) from public.audit_events where object_id in (
  select id from public.visit_plans where plan_reference like 'QA-PLAN-%'
  union all select id from public.visits where visit_reference like 'QA-VISIT-%'
  union all select id from public.assignments where id::text like '76000000-%'
  union all select id from public.inspections where id::text like '77000000-%'
  union all select id from public.submission_versions where idempotency_key like 'local-test-data-v1-%'
)
order by entity;

select r.role_key role,count(*) from public.profiles p join public.user_roles ur on ur.user_id=p.user_id join public.roles r on r.role_key=ur.role_key where p.email ~ '^(admin|planner|supervisor|inspector)[0-9]+@mim\.gov\.sa$' group by r.role_key order by r.role_key;
...
select user_id,array_agg(role_key order by role_key) roles from public.user_roles where user_id='a5000000-0000-4000-8000-000000000001' group by user_id;
select count(*) no_workspace_role_grants from public.user_roles where user_id='a6000000-0000-4000-8000-000000000001';
select count(*) filter(where encrypted_password is not null and encrypted_password like '$2%') credential_hashes_present,count(*) total from auth.users where email ~ '^(admin|planner|supervisor|inspector)[0-9]+@mim\.gov\.sa$';
select count(*) filter(where raw_app_meta_data->>'role'=regexp_replace(split_part(email,'@',1),'[0-9]+$','')) app_role_matches from auth.users where email ~ '^(admin|planner|supervisor|inspector)[1-5]@local[.]saqeel[.]test$';

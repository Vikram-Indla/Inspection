...
union all select 'notifications',count(*) from public.notifications where recipient in (select user_id from public.profiles where email like '%@local.saqeel.test')
...
select count(*) filter(where encrypted_password is not null and encrypted_password like '$2%') password_hashes_present,count(*) total from auth.users where email like '%@local.saqeel.test';

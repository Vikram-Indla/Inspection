...
for(const role of roles) for(let n=1;n<=5;n++){ const p={admin:'a1',planner:'a2',supervisor:'a3',inspector:'a4'}[role]; accounts.push([`${role}${n}`,`${role}${n}@local.saqeel.test`,role,id(p,n),'ENV_REQUIRED']); profiles.push([id(p,n),`${role}${n}`,names[role][n-1],regions[n-1],role==='admin'?'national':'regional','active']); }
accounts.push(['multi-role','multi-role@local.saqeel.test','planner + supervisor',id('a5',1),'ENV_REQUIRED'],['no-workspace','no-workspace@local.saqeel.test','none (refusal case)',id('a6',1),'ENV_REQUIRED']);
...
add('Import Order',['Step','Entity','Action','Rollback Order'],[['01','Environment','Guard loopback + production refusal','n/a'],['02','Roles','Verify/upsert governed four roles','16'],['03','Auth users','Upsert 22 deterministic local identities','15'],['04','Profiles and grants','Upsert profiles and role rows','14'],['05','Establishments and licences','Upsert master data','13'],['06','Packages and versions','Upsert form configuration','12'],['07','Plans and visits','Upsert planning graph','11'],['08','Assignments','Upsert inspector links','10'],['09','Inspection data','Upsert mutable execution fixtures','09'],['10','Validation','Counts, FKs, RLS, coordinates','n/a']]);
const val=add('Validation Reconciliation',['Check','Expected','Formula / Actual','Status'],[
 ['Accounts',22,null,null],['Profiles',22,null,null],['Establishments',5,null,null],['Licences',5,null,null],['Packages',5,null,null],['Plans',5,null,null],['Visits',5,null,null],['Assignments',5,null,null],['Submission versions',3,null,null],['Reviews',3,null,null],['Relationship cohorts',5,null,null],['Credential value cells',0,0,'PASS']]);
const refs=[['Accounts',25],['Profiles',25],['Establishments',8],['Licences',8],['Packages Templates',8],['Plans',8],['Visits',8],['Assignments',8],['Relationship Matrix',8]];
...
const summary=add('Import Summary',['Metric','Expected','Actual','Status'],[['Accounts',22,null,null],['Cohorts',5,null,null],['Factories',5,null,null],['Visits',5,null,null],['Core validation checks',12,null,null]],'Executive reconciliation for the local-only governed package.');

...
if(!workbookPath || !databaseUrl) throw new Error('Usage: LOCAL_TEST_DATABASE_URL=postgresql://... import_workbook.mjs workbook.xlsx');
...
const accountCheck=await wb.inspect({kind:'table',range:'Accounts!A3:F23',include:'values',tableMaxRows:25,tableMaxCols:8});
...
const run=spawnSync('psql',[databaseUrl,'-X','-v','ON_ERROR_STOP=1','-f',seedSql],{encoding:'utf8'});
if(run.status!==0) throw new Error(`Import failed: ${run.stderr.split('\n').slice(-6).join('\n')}`);
console.log(JSON.stringify({mode:'upsert',environment:'local',accounts:20,cohorts:5,seed_batch_id:'local-test-data-v1',status:'complete'}));

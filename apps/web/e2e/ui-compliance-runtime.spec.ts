...
for (const journey of journeys) {
  test.describe(`${journey.persona}: ${journey.label}`, () => {
    ...
    test(`UIC WCAG 2.2 AA, 320px reflow and targets — ${journey.locale}/${journey.theme}`, async ({ page }) => {
      ...
      const undersized = await page.locator("button:visible, input:not([type=checkbox]):not([type=radio]):visible, select:visible, textarea:visible, summary:visible, a:is(.sq-nav-item,.sq-nav-item):visible")

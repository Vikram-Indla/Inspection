...
test.describe("negative: authentication", () => {
  test("wrong password is denied with a safe banner, no session", async ({ page }) => {
    ...
    await identifierField(page).fill(PERSONAS.inspector.email);
    await passwordField(page).fill("WrongPassword!0000");
    await submitCredentials(page);
    const banner = page.locator('.fl-msg[role="alert"]');
    ...
    await expect(banner).not.toContainText("WrongPassword!0000");
  ...
  test("unknown account is denied with the same safe shape", async ({ page }) => {
    await page.goto("/login");
    await waitForCredentialsForm(page);
    await identifierField(page).fill("nobody@mim.gov.sa");
    await passwordField(page).fill("Whatever!2026");
    await submitCredentials(page);
    await expect(page.locator('.fl-msg[role="alert"]')).toBeVisible();
    await expect(page).toHaveURL(/\/login/);
  });

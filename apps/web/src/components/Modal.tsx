 ...
 * existing .sq-modal-backdrop/.sq-modal CSS with the behavior V1 call sites
...
export default function Modal({ open, onClose, titleId, title, descriptionId, description, children, footer, closeLabel, maxWidth }: ModalProps) {
  ...
  return (
    <div className="sq-modal-backdrop" onMouseDown={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div ref={dialogRef} className="sq-modal" role="dialog" aria-modal="true" aria-labelledby={titleId} aria-describedby={description ? descriptionId : undefined} style={maxWidth ? { inlineSize: `min(${maxWidth}, 100%)` } : undefined}>
        <div className="sq-modal__header sq-row" style={{ justifyContent: "space-between" }}>
          <h3 id={titleId} style={{ margin: 0 }}>{title}</h3>
          <button ref={closeBtnRef} type="button" className="sq-btn sq-btn--secondary sq-btn--icon" aria-label={closeLabel} onClick={onClose}>
            <IconClose size={16} />
          </button>
        </div>
        <div className="sq-modal__body">
          {description ? <p id={descriptionId} className="sq-caption">{description}</p> : null}
          {children}
        </div>
        {footer ? <div className="sq-modal__footer">{footer}</div> : null}
      </div>
    </div>
  );
}

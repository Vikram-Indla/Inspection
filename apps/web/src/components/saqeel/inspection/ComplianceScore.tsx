...
export function ComplianceScore({ value = 0, size = 64, label = "Compliance" }: ComplianceScoreProps) {
    ...
    <div className="row" style={{ gap: "var(--space-3)" }}>
      <svg width={size} height={size} role="img" aria-label={label + " " + value + "%"}>
        ...
        <text
          ...
          textAnchor="middle"
          style={{ font: "600 " + size / 4 + "px var(--font-body)", fill: "var(--text-primary)" }}
        >
          {value}%
        </text>
      </svg>
      <div className="stack" style={{ gap: 2 }}>
        ...
        <span className={"badge badge-" + tone}>
          ...
          {tone === "compliant" ? "Compliant" : tone === "warning" ? "Needs attention" : "Non-compliant"}
        </span>
      </div>
    </div>
  );

...
export function ChecklistQuestion({
  ...
  return (
    <div
      className="stack"
      style={{ gap: "var(--space-2)", paddingBlock: "var(--space-3)", borderBlockEnd: "1px solid var(--border-subtle)" }}
    >
      <div className="row" style={{ alignItems: "flex-start" }}>
        <span className="t-label" style={{ fontVariantNumeric: "tabular-nums", minWidth: 28 }}>
          {number}
        </span>
        <div className="grow">
          <div className="t-compact" style={{ fontWeight: 500 }}>
            {question}
          </div>
      ...
      <div className="row" style={{ paddingInlineStart: 28, flexWrap: "wrap" }}>
        ...
        {onNote && (
          <button type="button" className="btn btn-ghost btn-sm" onClick={onNote}>
            {note ? "Edit note" : "Add note"}
          </button>
        )}
        {onEvidence && (
          <button type="button" className="btn btn-ghost btn-sm" onClick={onEvidence}>
            Evidence{evidenceCount ? " (" + evidenceCount + ")" : ""}
          </button>
        )}
      </div>
      {note && (
        <div className="t-caption" style={{ paddingInlineStart: 28 }}>
          {note}
        </div>
      )}
    </div>

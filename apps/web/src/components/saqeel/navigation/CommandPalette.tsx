...
export function CommandPalette({
  ...
  onClose,
  items = [],
  placeholder = "Search inspections, facilities, actions…",
}: CommandPaletteProps) {
  ...
  const onKey = (e: React.KeyboardEvent) => {
    if (e.key === "ArrowDown") {
      ...
      onClose && onClose();
      flat[active].onSelect && flat[active].onSelect!();
    }
  };
  let idx = -1;
  return (
    <div
      ...
      style={{ alignItems: "start", paddingBlockStart: "12vh" }}
    ...
    >
      <div className="palette" role="dialog" aria-modal="true" aria-label="Command palette">
        ...
        <div className="palette-list">
          {flat.length === 0 && (
            <div className="t-meta" style={{ padding: "var(--space-4)", textAlign: "center" }}>
              No results for “{q}”
            </div>
          )}
          {sections.map((s) => (

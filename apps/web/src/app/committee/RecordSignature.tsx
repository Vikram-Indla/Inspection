...
export function RecordSignature({ strings: s }: { strings: { record: string; recording: string; recorded: string; kind: string; outcome: string } }) {
  ...
  return (
    <form action={action} className="ax-surface" style={{ padding: "var(--ax-space-300)", display: "flex", gap: "var(--ax-space-150)", alignItems: "flex-end", flexWrap: "wrap" }}>
      <div className="ax-field"><label className="ax-field__label">{s.kind}</label>
        <select className="ax-input" name="kind"><option value="signature">signature</option><option value="refusal">refusal</option></select></div>
      <div className="ax-field"><label className="ax-field__label">{s.outcome}</label>
        <select className="ax-input" name="outcome"><option value="captured">captured</option><option value="refused">refused</option><option value="queued">queued</option><option value="failed">failed</option></select></div>

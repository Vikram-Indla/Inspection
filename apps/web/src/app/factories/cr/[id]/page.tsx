      ...
      <div className={styles.workspace} data-factory360-layout="cr-license-dossier">
        ...
        <main className={styles.main}>
          ...
          <section className={`ax-surface ${styles.panel}`} aria-labelledby="f360-compliance-heading">
            ...
            {approvedTrend.length > 1 && <div className={styles.trend} aria-label={t("f360.compliance.trend", "Approved compliance trend")}>{[...approvedTrend].reverse().map(({ report, compliance }) => <div className={styles.trendPoint} key={report.id} title={`${text(report.inspection_no)} · ${compliance.rate}%`}><span className={styles.trendBar} style={{ blockSize: `${Math.max(4, compliance.rate ?? 0)}px` }} /><span className="t-caption ax-numeric">{compliance.rate}%</span></div>)}</div>}
          </section>
          ...
          <section className={`ax-surface ${styles.panel}`} aria-labelledby="f360-government-heading">
            ...
            {government.length ? <div className="ax-tablewrap"><table className="ax-table"><thead><tr><th scope="col">{t("common.type", "Type")}</th><th scope="col">{t("common.reference", "Reference")}</th><th scope="col">{t("common.status", "Status")}</th><th scope="col">{t("f360.validity", "Validity")}</th><th scope="col">{t("f360.source", "Source")}</th></tr></thead><tbody>{government.map(row => <tr key={row.id}><td>{text(row.title ?? row.record_type)}</td><td className="ax-numeric"><bdi>{row.external_record_id}</bdi></td><td>{label(row.status)}</td><td className="ax-numeric">{dt(row.valid_from)} → {dt(row.valid_to)}</td><td>{row.source_system} · v{row.version_number}</td></tr>)}</tbody></table></div> : <p className="t-caption">{governmentResult.error ? t("f360.section.degraded", "This source section is degraded; other sections remain available.") : t("f360.government.empty", "Government-domain records are unavailable until a governed source contract supplies them.")}</p>}
          </section>
          ...
          <section className={`ax-surface ${styles.panel}`} aria-labelledby="f360-docs-heading">
            ...
            {!permissions["view_factory_documents"] ? <p className="t-caption">{t("f360.documents.restricted", "Document metadata requires Factory Documents permission.")}</p> : <>
              {docs.length ? <div className="ax-tablewrap"><table className="ax-table"><thead><tr><th scope="col">{t("common.category", "Category")}</th><th scope="col">{t("common.title", "Title")}</th><th scope="col">{t("common.reference", "Reference")}</th><th scope="col">{t("f360.validity", "Validity")}</th><th scope="col">{t("f360.source", "Source")}</th><th scope="col" /></tr></thead><tbody>{docs.map(doc => <tr key={doc.id}><td>{label(doc.business_category ?? doc.doc_type)}</td><td>{doc.title}</td><td className="ax-numeric"><bdi>{text(doc.reference_no)}</bdi></td><td className="ax-numeric">{dt(doc.valid_from)} → {dt(doc.valid_to)}</td><td>{text(doc.source_system)} · {text(doc.source_status)}</td><td>{permissions["download_factory_documents"] && downloadUrls[doc.id] ? <a className="ax-link" href={downloadUrls[doc.id]} download>{t("common.download", "Download")}</a> : <span className="t-caption">{permissions["download_factory_documents"] ? t("f360.download.unavailable", "file unavailable") : t("f360.download.restricted", "download restricted")}</span>}</td></tr>)}</tbody></table></div> : <p className="t-caption">{t("f360.documents.empty", "No source-backed document metadata is available.")}</p>}

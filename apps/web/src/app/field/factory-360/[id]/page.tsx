  ...
  return (
    <Shell current="/field" title={crTitle}
      ...
      <div className={`ax-field-page ${styles.page}`} data-factory360-layout="ipad-field">
        ...
        <section className={`ax-surface ${styles.header}`} aria-label={t("f360.context.heading", "Selected context")}>
          <div className={styles.headerTop}>
            <h2 className={styles.headerName}>{crTitle}</h2>
            {badge(dossier.crError, cr)}
          </div>
          <div className={styles.ids}>
            <span><strong>{t("f360.id.cr", "CR")}</strong> <bdi className="ax-numeric">{cr.cr_number}</bdi></span>
            {cr.unified_number && <span><strong>{t("f360.cr.unified", "Unified")}</strong> <bdi className="ax-numeric">{cr.unified_number}</bdi></span>}
            <span><strong>{t("f360.id.license", "License")}</strong> <bdi className="ax-numeric">{text(selected?.license_number)}</bdi></span>
        ...
        <section className={`ax-surface ${styles.header}`} aria-label={t("f360.licenses.heading", "Industrial licenses and plants")}>
          ...
          {licenses.length ? <div className={styles.licenseStrip}>{licenses.map(row => (
            <a key={row.id} className={styles.licenseChip} href={withLicense(row.id)} aria-current={row.id === selected?.id ? "page" : undefined}>
              ...
              <span className="ax-caption">{row.factories?.name ?? "—"}</span>
            </a>
        ...
        <section className={`ax-surface`} aria-label={t("f360.readiness.heading", "Field readiness")}>
          <div className={styles.readiness}>
            <div className={`ax-surface ${styles.tile}`}>
              <span className="ax-caption">{t("f360.risk.heading", "Saved risk")}</span>
              <span className={`${styles.tileValue} ax-numeric`}>{permissions["view_risk_details"] ? text(factory?.risk_score) : t("f360.restricted", "restricted")}</span>
              <span className="ax-caption">{permissions["view_risk_details"] ? label(factory?.risk_band) : ""}</span>
            </div>
            <div className={`ax-surface ${styles.tile}`}>
              <span className="ax-caption">{t("f360.compliance.heading", "Approved compliance")}</span>
              <span className={`${styles.tileValue} ax-numeric`}>{currentCompliance.rate == null ? t("f360.compliance.notAvailable", "Not Available") : `${currentCompliance.rate}%`}</span>
              <span className="ax-caption">{currentCompliance.status === "available" ? `${currentCompliance.passed}/${currentCompliance.answered}` : ""}</span>
            </div>
            <div className={`ax-surface ${styles.tile}`}>
              <span className="ax-caption">{t("f360.cr.approvedInspections", "Approved inspections")}</span>
              <span className={`${styles.tileValue} ax-numeric`}>{portfolioReportsResult.error ? t("f360.source.degraded", "degraded") : portfolioCounts.approvedInspections}</span>
            </div>
            <div className={`ax-surface ${styles.tile}`}>
              <span className="ax-caption">{t("f360.location", "Location")}</span>
        ...
        {/* Factory profile */}
        <details className={`ax-surface ${styles.section}`} open>
        ...
        <details className={`ax-surface ${styles.section}`}>
          ...
          <div className={styles.sectionBody}>
            {!permissions["view_risk_details"] ? <p className="ax-caption">{t("f360.risk.restricted", "Risk detail requires Factory Risk permission.")}</p> : <>
              ...
              <p className="ax-caption">{t("f360.risk.version", "Model")} {text(factory?.risk_version)} · {dt(factory?.risk_calculated_at)} · {riskHistory.length} {t("f360.risk.snapshots", "saved snapshots")}</p>
        ...
        <details className={`ax-surface ${styles.section}`}>
          ...
          <div className={styles.sectionBody}>
            {lines.length ? <div className="ax-tablewrap"><table className="ax-table"><thead><tr><th scope="col">{t("common.type", "Type")}</th><th scope="col">{t("common.name", "Name")}</th><th scope="col">{t("f360.hsCode", "HS / activity")}</th><th scope="col">{t("f360.quantity", "Qty / cap")}</th><th scope="col">{t("f360.production", "Real / max")}</th></tr></thead><tbody>{lines.map(row => <tr key={row.id}><td>{label(row.item_type)}</td><td>{showLineName(row)}</td><td className="ax-numeric"><bdi>{text(row.hs_code ?? row.activity_code)}</bdi></td><td className="ax-numeric">{text(row.quantity)} / {text(row.capacity)}</td><td className="ax-numeric">{text(row.real_production)} / {text(row.maximum_production)}</td></tr>)}</tbody></table></div> : <p className="ax-caption">{linesResult.error ? t("f360.section.degraded", "This source section is degraded; other sections remain available.") : t("f360.industrial.empty", "No source-backed products, spare parts, machines, production lines or raw materials are available.")}</p>}
            ...
            {latestApprovedFactorySnapshot ? <>
              ...
              <p className="ax-caption">{t("f360.observed.rule", "The observed column is the immutable official hierarchy snapshot captured with a submission that later became an approved report. It does not overwrite current source truth.")} {snapshotOrigin ? <a className="ax-link" href={`/reports/inspection/${snapshotOrigin.id}`}>{t("f360.observed.origin", "origin approved report")}</a> : null}</p>
        ...
        <details className={`ax-surface ${styles.section}`}>
          ...
          <div className={styles.sectionBody}>
            {government.length ? <div className="ax-tablewrap"><table className="ax-table"><thead><tr><th scope="col">{t("common.type", "Type")}</th><th scope="col">{t("common.reference", "Reference")}</th><th scope="col">{t("common.status", "Status")}</th><th scope="col">{t("f360.validity", "Validity")}</th></tr></thead><tbody>{government.map(row => <tr key={row.id}><td>{text(row.title ?? row.record_type)}</td><td className="ax-numeric"><bdi>{row.external_record_id}</bdi></td><td>{label(row.status)}</td><td className="ax-numeric">{dt(row.valid_from)} → {dt(row.valid_to)}</td></tr>)}</tbody></table></div> : <p className="ax-caption">{governmentResult.error ? t("f360.section.degraded", "This source section is degraded; other sections remain available.") : t("f360.government.empty", "Government-domain records are unavailable until a governed source contract supplies them.")}</p>}
          </div>
        </details>
        ...
        <details className={`ax-surface ${styles.section}`}>
          ...
          <div className={styles.sectionBody}>
            {!permissions["view_factory_documents"] ? <p className="ax-caption">{t("f360.documents.restricted", "Document metadata requires Factory Documents permission.")}</p> : <>
              {docs.length ? <ul>{docs.map(doc => <li key={doc.id}>{label(doc.business_category ?? doc.doc_type)} · {doc.title} · <bdi>{text(doc.reference_no)}</bdi> {permissions["download_factory_documents"] && downloadUrls[doc.id] ? <a className="ax-link" href={downloadUrls[doc.id]} download>{t("common.download", "Download")}</a> : <span className="ax-caption">{permissions["download_factory_documents"] ? t("f360.download.unavailable", "file unavailable") : t("f360.download.restricted", "download restricted")}</span>}</li>)}</ul> : <p className="ax-caption">{t("f360.documents.empty", "No source-backed document metadata is available.")}</p>}
              {officialMedia.some(asset => mediaUrls[asset.id]) && <><h3>{t("f360.media.official", "Official factory gallery")}</h3><div className={styles.mediaGrid}>{officialMedia.filter(asset => mediaUrls[asset.id]).map(asset => <figure key={asset.id}><img src={mediaUrls[asset.id]} alt={asset.title ?? t("f360.media.alt", "Official factory image")} /><figcaption className="ax-caption">{asset.title ?? label(asset.category)}</figcaption></figure>)}</div></>}
        ...
        <details className={`ax-surface ${styles.section}`}>
          ...
          <div className={styles.sectionBody}>
            <p className="ax-caption">{t("f360.xpc.rule", "Facts are resolved once, server-side. Industry Shared master data is not shown until it can be confirmed; Inspection API values are contextual; approved report facts are authoritative. Conflicts are surfaced, never overwritten.")}</p>
        ...
        <div className={`ax-surface ${styles.actionBar}`} role="group" aria-label={t("common.actions", "Actions")}>
          ...
          {factoryId && <a className="ax-btn ax-btn--secondary" href={`/planning/single?cr=${encodeURIComponent(cr.cr_number)}&license=${encodeURIComponent(selected?.license_number ?? "")}&plant=${encodeURIComponent(selected?.plant_number ?? "")}&factory=${factoryId}&source=factory360`}>{t("f360.actions.planSingle", "Plan single visit")}</a>}

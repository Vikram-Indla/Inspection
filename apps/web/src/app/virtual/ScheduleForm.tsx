...
export default function ScheduleForm({ visitId, strings }: { visitId: string; strings: ScheduleFormStrings }) {
  ...
  return (
    <form action={formAction} className="ax-row" style={{ alignItems: "flex-end" }}>
      ...
      <div className="ax-field"><label className="ax-field__label">{strings.appointment}</label>
        <input className="ax-input ax-numeric" type="datetime-local" name="appointment_at" required /></div>
      <div className="ax-field"><label className="ax-field__label">{strings.repName}</label>
        <input className="ax-input" name="rep_name" placeholder={strings.repNamePh} required /></div>

...
export default async function EnforcementPage() {
  ...
  return (
    <Shell current="/enforcement" title={t("mvp3.enforcement.title", "Enforcement and correction lifecycle")}
      ...
      <section className="ax-surface stack" style={{ padding: "var(--ax-space-300)", marginBlockStart: "var(--ax-space-200)" }}><h3>{t("mvp3.enforcement.objectionQueue", "Correction and objection queue")}</h3>{(objections ?? []).map(row => <div className="ax-row" key={row.id} style={{ justifyContent: "space-between", flexWrap: "wrap" }}><span><strong>{row.grounds}</strong><small className="ax-caption"> · {new Date(row.created_at).toLocaleDateString()}</small></span><span className="ax-lozenge">{row.status}</span></div>)}{!(objections ?? []).length ? <p className="ax-caption">{t("mvp3.enforcement.noObjections", "No RLS-visible objections.")}</p> : null}</section>
    </Shell>
  );
}

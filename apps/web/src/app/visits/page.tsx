  ...
  return (
    <Shell current="/visits" title={t("visit.list.title", "Visit management")}
      ...
      <div className="row" style={{ justifyContent: "space-between", flexWrap: "wrap", alignItems: "center", gap: "var(--ax-space-150)" }}>
        ...
        <span className="t-caption ax-numeric">{t("visit.list.scope", "RLS-scoped — showing {shown} of {total}").replace("{shown}", String(Math.min(rows.length, limit))).replace("{total}", String(total))}</span>
      </div>

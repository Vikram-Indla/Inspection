  ...
  return (
    <Shell current="/admin/integrations" title={t("mvp3.integrations.title", "Integration trust console")}
      ...
      <section className="ax-surface stack" style={{ padding: "var(--ax-space-300)" }} aria-labelledby="integration-registry">
        <div className="row" style={{ justifyContent: "space-between", flexWrap: "wrap" }}><div><h3 id="integration-registry">{t("mvp3.integrations.registry", "Governed endpoint registry")}</h3><p className="t-caption">{t("mvp3.integrations.registryHelp", "Contract version, runtime state and dependency truth—not secret material.")}</p></div><span className="ax-lozenge">{(endpoints ?? []).length} endpoints</span></div>
      ...
      <div className="ax-grid" style={{ marginBlockStart: "var(--ax-space-200)" }}>
        <section className="ax-surface stack" style={{ padding: "var(--ax-space-300)" }}><h3>{t("mvp3.integrations.events", "API and rule events")}</h3><p className="t-caption">{t("mvp3.integrations.eventsHelp", "Append-only outcomes linked by correlation ID.")}</p>{(events ?? []).slice(0, 8).map(e => <div className="row" key={e.id} style={{ justifyContent: "space-between" }}><span><strong>{e.event_kind}</strong><small className="t-caption"> · {e.direction}</small></span><span className="ax-lozenge">{e.outcome}</span></div>)}{!(events ?? []).length ? <p className="t-caption">{t("mvp3.integrations.noEvents", "No RLS-visible events.")}</p> : null}</section>
        <section className="ax-surface stack" style={{ padding: "var(--ax-space-300)" }}><h3>{t("mvp3.integrations.exports", "Data-sharing exports")}</h3><p className="t-caption">{t("mvp3.integrations.exportsHelp", "Prepared is not delivered. Delivery requires an artifact hash and receipt.")}</p>{(exports ?? []).slice(0, 8).map(e => <div className="row" key={e.id} style={{ justifyContent: "space-between" }}><span><strong>{e.export_kind}</strong><small className="t-caption"> · {e.purpose}</small></span><span className="ax-lozenge">{e.status}</span></div>)}{!(exports ?? []).length ? <p className="t-caption">{t("mvp3.integrations.noExports", "No RLS-visible export jobs.")}</p> : null}</section>
      </div>
    </Shell>
  );
}

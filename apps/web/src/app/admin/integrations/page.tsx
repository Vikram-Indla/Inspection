  ...
  return (
    <Shell current="/admin/integrations" title={t("mvp3.integrations.title", "Integration trust console")}
      ...
      <div className="ax-grid" style={{ marginBlockStart: "var(--ax-space-200)" }}>
        <section className="ax-surface stack" style={{ padding: "var(--ax-space-300)" }}><h3>{t("mvp3.integrations.events", "API and rule events")}</h3><p className="t-caption">{t("mvp3.integrations.eventsHelp", "Append-only outcomes linked by correlation ID.")}</p>{(events ?? []).slice(0, 8).map(e => <div className="row" key={e.id} style={{ justifyContent: "space-between" }}><span><strong>{e.event_kind}</strong><small className="t-caption"> · {e.direction}</small></span><span className="badge">{e.outcome}</span></div>)}{!(events ?? []).length ? <p className="t-caption">{t("mvp3.integrations.noEvents", "No RLS-visible events.")}</p> : null}</section>

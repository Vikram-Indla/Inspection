...
function Result({ state }: { state: FactoryDataResult }) {
  ...
  if (state.ok) return <span className="ax-lozenge ax-lozenge--success">Saved and audit-recorded</span>;
  return null;
}
...
function RepresentativeToggle({ factoryId, representative }: { factoryId: string; representative: { id: string; full_name: string; active: boolean } }) {
  ...
  return <form action={action} className="ax-row" style={{ justifyContent: "space-between" }}>
    ...
    <button className="ax-btn ax-btn--subtle" disabled={pending}>{representative.active ? "Deactivate" : "Reactivate"}</button><Result state={state} />
  </form>;
}
...
export default function MasterDataForms({ factoryId, representatives }: { factoryId: string; representatives: { id: string; full_name: string; active: boolean }[] }) {
  ...
  return <><div className="ax-grid">
    <form action={documentAction} className="ax-surface ax-stack" style={{ padding: "var(--ax-space-200)" }}>
      ...
      <label>Reference number<input className="ax-input ax-numeric" name="reference_no" /></label>
      <label>Valid from<input className="ax-input ax-numeric" type="date" name="valid_from" /></label><label>Valid to<input className="ax-input ax-numeric" type="date" name="valid_to" /></label>
      <button className="ax-btn ax-btn--secondary" disabled={documentPending}>Add metadata</button><Result state={documentState} />
    </form>
    <form action={representativeAction} className="ax-surface ax-stack" style={{ padding: "var(--ax-space-200)" }}>
      ...
      <button className="ax-btn ax-btn--secondary" disabled={representativePending}>Add representative</button><Result state={representativeState} />
    </form>
    <form action={productAction} className="ax-surface ax-stack" style={{ padding: "var(--ax-space-200)" }}>
      ...
      <label>Annual capacity<input className="ax-input ax-numeric" type="number" min="0" step="0.01" name="annual_capacity" /></label><label className="ax-choice"><input type="checkbox" name="is_primary" /> Primary product</label>
      <button className="ax-btn ax-btn--secondary" disabled={productPending}>Add product</button><Result state={productState} />
    </form>
    <form action={materialAction} className="ax-surface ax-stack" style={{ padding: "var(--ax-space-200)" }}>
      ...
      <button className="ax-btn ax-btn--secondary" disabled={materialPending}>Add material</button><Result state={materialState} />
    </form>
  </div><section className="ax-surface ax-stack" style={{ padding: "var(--ax-space-200)", marginBlockStart: "var(--ax-space-200)" }}><h4>Representative status</h4>

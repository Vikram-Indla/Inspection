...
export default function AuditReplayWorkspace(props: Props) {
  ...
  if (!props.authorized) return <section className="ax-surface ar-denied" role="alert"><span aria-hidden="true">🛡</span><h2>{L.unauthorized}</h2><p>RBAC / RLS · zero disclosure</p></section>;
  ...
  return <div className="ar-workspace" dir={props.locale === "ar" ? "rtl" : "ltr"}>
    ...
    <nav className="ar-modes" aria-label="Audit replay modes">{modes.map(([id,label]) => <a key={id} className={`ax-btn ${props.mode === id ? "ax-btn--prominent" : "ax-btn--secondary"}`} aria-current={props.mode === id ? "page" : undefined} href={`?view=${id}&case=${encodeURIComponent(props.caseRef)}&q=${encodeURIComponent(props.query)}`}>{label}</a>)}</nav>
    {props.partialScope && <div className="ax-banner" role="status"><div><strong>PARTIAL SCOPE.</strong> {L.partial}</div></div>}
    {props.semanticUnavailable && <div className="ax-banner ax-banner--warning" role="status"><div><strong>DEGRADED.</strong> {L.degraded}</div></div>}
    ...
    {props.mode === "recorder" && props.events.length > 0 && <section id="audit-chronology" className="ar-recorder">
      ...
      <aside className="ar-dossier ax-surface"><h3>{L.dossier}</h3><p>{atState.length ? String(atState.length) : L.noEvent}</p><dl><dt>{L.governing}</dt><dd>{L.missing}</dd><dt>{L.correlation}</dt><dd>{props.events.some(e => e.correlationId) ? "PARTIAL" : L.missing}</dd><dt>{L.legal}</dt><dd>POLICY_HELD · DEC-006 / DEC2-009</dd></dl></aside>
    </section>}

    {props.mode === "reconstruct" && <section className="ax-surface ar-modepanel"><form method="get"><input type="hidden" name="view" value="reconstruct"/><input type="hidden" name="case" value={props.caseRef}/><label className="ax-field"><span className="ax-field__label">{L.at}</span><input className="ax-input" type="datetime-local" name="at" defaultValue={props.at?.slice(0,16)}/></label><button className="ax-btn ax-btn--prominent">{L.apply}</button></form><h3>{atState.length} reconstructed aggregate states</h3>{atState.map(row => <article key={row.key} className="ar-custody"><strong>{row.key}</strong><span>{row.eventIds.length} source events · last {row.lastOccurredAt}</span>{row.conflicts.length > 0 && <span className="ax-lozenge ax-lozenge--critical">CONFLICT</span>}<pre>{json(row.state)}</pre></article>)}</section>}

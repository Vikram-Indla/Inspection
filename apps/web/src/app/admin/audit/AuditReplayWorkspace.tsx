...
export default function AuditReplayWorkspace(props: Props) {
  ...
  return <div className="ar-workspace" dir={props.locale === "ar" ? "rtl" : "ltr"}>
    ...
    {props.mode === "reconstruct" && <section className="panel ar-modepanel"><form method="get"><input type="hidden" name="view" value="reconstruct"/><input type="hidden" name="case" value={props.caseRef}/><label className="ax-field"><span className="ax-field__label">{L.at}</span><input className="ax-input" type="datetime-local" name="at" defaultValue={props.at?.slice(0,16)}/></label><button className="btn btn-primary btn-lg btn-touch">{L.apply}</button></form><h3>{atState.length} reconstructed aggregate states</h3>{atState.map(row => <article key={row.key} className="ar-custody"><strong>{row.key}</strong><span>{row.eventIds.length} source events · last {row.lastOccurredAt}</span>{row.conflicts.length > 0 && <span className="badge badge-critical">{L.conflictTag}</span>}<pre>{json(row.state)}</pre></article>)}</section>}
    {props.mode === "compare" && <section className="panel ar-modepanel"><form method="get" className="ar-compareform"><input type="hidden" name="view" value="compare"/><input type="hidden" name="case" value={props.caseRef}/><label className="ax-field"><span className="ax-field__label">{L.at}</span><input className="ax-input" type="datetime-local" name="at" defaultValue={props.at?.slice(0,16)}/></label><label className="ax-field"><span className="ax-field__label">{L.vs}</span><input className="ax-input" type="datetime-local" name="vs" defaultValue={props.vs?.slice(0,16)}/></label><button className="btn btn-primary btn-lg btn-touch">{L.apply}</button></form>{comparison.map(row => <article key={row.key}><h3>{row.key} · {row.changed ? "CHANGED" : "UNCHANGED"}</h3><div className="ar-diff"><div><h4>{L.before}</h4><pre>{json(row.before?.state)}</pre></div><div><h4>{L.after}</h4><pre>{json(row.after?.state)}</pre></div></div></article>)}</section>}

...
export default async function ComplianceApprovalQueue({
  ...
  return (
    <Shell current="/admin/compliance-approvals" title="Compliance Approval Queue" context={<><span className="badge badge-info">CCR maker-checker</span><span className="t-caption">Distinct from Inspection Review &amp; Approval</span></>}>
      ...
      {readFailed ? <div className="panel"><div className="ax-state" role="alert"><span className="ax-state__glyph" aria-hidden="true">⚠</span><h4>Approval Queue unavailable</h4><p className="t-caption">One or more governed reads failed. Workload, progress, and readiness have not been inferred.</p><a className="ax-link" href={`/admin/compliance-approvals?view=${filterKey}`}>Retry queue</a></div></div> : rows.length === 0 ? <div className="panel"><div className="ax-state" role="status"><span className="ax-state__glyph" aria-hidden="true">✓</span><h4>No {filter.label.toLowerCase()} requests in your scope</h4><p className="t-caption">The RLS-scoped read succeeded and returned zero eligible maker-checker assignments for this view.</p></div></div> : <div className="cmp-approval-list">{rows.map(row => {

...
export default async function InspectorRuntimePreview({
  ...
  if (itemError || !item) {
    return <Shell current="/admin/items" title="Inspector Runtime Preview"><div className="ax-surface"><div className="ax-state" role="alert"><span className="ax-state__glyph" aria-hidden="true">🔎</span><h4>{itemError ? "Preview unavailable" : "Inspection item not found"}</h4><p className="t-caption">{itemError ? "The configuration read failed. Retry without assuming the item is absent." : "The read succeeded, but no item has this identifier."}</p><a className="ax-link" href="/admin/items">Back to Compliance Library</a></div></div></Shell>;
  }

...
function Feedback({ state, saved }: { state: TemplateResult; saved: string }) {
  return state.error ? <span className="t-caption" style={{ color: "var(--ax-color-critical)" }} role="alert">{state.error}</span> : state.ok ? <span className="badge badge-compliant" role="status">✓ {saved}</span> : null;
}
...
export default function TemplateRegistry({ templates, strings: s }: { templates: TemplateRow[]; strings: TemplateStrings }) {
  ...
  return <details className="panel stack" style={{ padding: "var(--ax-space-300)" }}>
    ...
    <form action={createAction} className="ax-grid-2">
      <label className="ax-field"><span className="ax-field__label">{s.key}</span><input className="ax-input numeric" name="template_key" required /></label>
      ...
      <label className="ax-field"><span className="ax-field__label">{s.version}</span><input className="ax-input numeric" name="version_label" placeholder="v1" required /></label>
      ...
      <label className="ax-field" style={{ gridColumn: "1 / -1" }}><span className="ax-field__label">{s.schema}</span><textarea className="ax-input numeric" name="schema" defaultValue={'{"fields":[]}'} required /></label>

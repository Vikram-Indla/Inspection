...
function Feedback({ state, saved }: { state: TemplateResult; saved: string }) {
  return state.error ? <span className="t-caption" style={{ color: "var(--ax-color-critical)" }} role="alert">{state.error}</span> : state.ok ? <span className="ax-lozenge ax-lozenge--success" role="status">✓ {saved}</span> : null;
}

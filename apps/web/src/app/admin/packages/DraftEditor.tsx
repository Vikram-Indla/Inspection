...
export default function DraftEditor({ versionId, definition, catalog, violations, templates, strings: s, preview }: {
  ...
  return <form action={formAction} className={styles.editorForm} aria-labelledby={`designer-${versionId}`}>
    ...
    <div className={styles.versionHeading}><h4 id={`designer-${versionId}`} style={{ margin: 0 }}>{s.heading}</h4><span className="row" style={{ gap: "var(--ax-space-100)", flexWrap: "wrap" }}>{dirty && <span className="badge badge-warning" role="status">● {s.unsaved}</span>}<span className="badge badge-info">✎ {s.editableWhileDraft}</span></span></div>

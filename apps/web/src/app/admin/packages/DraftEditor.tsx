...
export default function DraftEditor({ versionId, definition, catalog, violations, templates, strings: s, preview }: {
  ...
  return <form action={formAction} className={styles.editorForm} aria-labelledby={`designer-${versionId}`}>
    ...
    <div className={styles.versionHeading}><h4 id={`designer-${versionId}`} style={{ margin: 0 }}>{s.heading}</h4><span className="ax-row" style={{ gap: "var(--ax-space-100)", flexWrap: "wrap" }}>{dirty && <span className="ax-lozenge ax-lozenge--warning" role="status">● {s.unsaved}</span>}<span className="ax-lozenge ax-lozenge--info">✎ {s.editableWhileDraft}</span></span></div>
    <div className={styles.designerStudio}>
      <aside className={`${styles.studioPane} ${styles.editorCanvas}`}><h5 style={{ margin: 0 }}>{s.structure}</h5>
        {sections.length === 0 ? <div className="ax-state"><strong>{s.emptyTitle}</strong><p className="ax-caption">{s.emptyBody}</p></div> : <ol className={styles.structureList}>{sections.map((section, index) => <li key={section.key}><button type="button" className={styles.structureButton} aria-current={section.key === selected?.key ? "true" : undefined} onClick={() => { setSelectedKey(section.key); setSelectedItem(""); }}><strong>{index + 1}. {section.title_en ?? section.title ?? section.key}</strong><br/><span className="ax-caption">{section.title_ar ?? "—"} · {fill(s.itemCount, { n: section.items?.length ?? 0 })}</span></button></li>)}</ol>}
        ...
        <button type="button" className="ax-btn" disabled={!newSectionEn.trim() || !newSectionAr.trim()} onClick={addSection}>{s.addSection}</button>
      </aside>
      <section className={`${styles.studioPane} ${styles.editorCanvas}`}><h5 style={{ margin: 0 }}>{s.fieldCanvas}</h5>
        {selected ? <>
          <div className="ax-row" style={{ gap: 8 }}><button type="button" className="ax-btn" onClick={() => moveSection(-1)} disabled={selectedIndex === 0}>{s.moveUp}</button><button type="button" className="ax-btn" onClick={() => moveSection(1)} disabled={selectedIndex === sections.length - 1}>{s.moveDown}</button></div>
          ...
          <ol className="ax-stack" style={{ paddingInlineStart: 20 }}>{(selected.items ?? []).map((code, index) => <li key={code}><div className="ax-row" style={{ gap: 6, flexWrap: "wrap" }}><button type="button" className="ax-btn ax-btn--subtle" onClick={() => setSelectedItem(code)}><bdi dir="ltr">{code}</bdi></button><button type="button" className="ax-btn" aria-label={`${s.moveUp} ${code}`} disabled={index === 0} onClick={() => moveItem(code, -1)}>↑</button><button type="button" className="ax-btn" aria-label={`${s.moveDown} ${code}`} disabled={index === (selected.items?.length ?? 0) - 1} onClick={() => moveItem(code, 1)}>↓</button><button type="button" className="ax-btn" aria-label={`${s.removeAria} ${code}`} onClick={() => { patchSelected({ items: (selected.items ?? []).filter(item => item !== code) }); setSelectedItem(""); }}>✕</button></div></li>)}</ol>
        ...
        {selectedItem && rule && <fieldset className="ax-panel ax-stack" style={{ padding: 16 }}><legend><strong>{s.relationship}: <bdi dir="ltr">{selectedItem}</bdi></strong></legend>
        ...
        <div className="ax-panel" style={{ padding: 12 }}><strong>{s.validationTitle}</strong>{validationIssues.length ? <ul className="ax-caption" role="alert">{validationIssues.map(issue => <li key={issue}>{issue}</li>)}</ul> : <p className="ax-caption" role="status">✓ {s.validationClear} {s.circularCheck}</p>}</div>

  ...
  return <form action={formAction} className={styles.editorForm} aria-labelledby={`designer-${versionId}`}>
    ...
    <div className={styles.designerStudio}>
      ...
      <section className={`${styles.studioPane} ${styles.editorCanvas}`}><h5 style={{ margin: 0 }}>{s.fieldCanvas}</h5>
        ...
        <div className={styles.actionRow}>{!dirty && !state.ok && <span className="t-caption">{s.noChanges}</span>}<button className="ax-btn ax-btn--prominent" disabled={pending || !dirty || validationIssues.length > 0}>{pending ? s.saving : s.save}</button></div>
      </section>

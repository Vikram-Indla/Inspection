    ...
    <div className={styles.designerStudio}>
      ...
      <section className={`${styles.studioPane} ${styles.editorCanvas}`}><h5 style={{ margin: 0 }}>{s.fieldCanvas}</h5>
        {selected ? <>
          ...
          <ol className="stack" style={{ paddingInlineStart: 20 }}>{(selected.items ?? []).map((code, index) => <li key={code}><div className="ax-row" style={{ gap: 6, flexWrap: "wrap" }}><button type="button" className="ax-btn ax-btn--subtle" onClick={() => setSelectedItem(code)}><bdi dir="ltr">{code}</bdi></button><button type="button" className="ax-btn" aria-label={`${s.moveUp} ${code}`} disabled={index === 0} onClick={() => moveItem(code, -1)}>↑</button><button type="button" className="ax-btn" aria-label={`${s.moveDown} ${code}`} disabled={index === (selected.items?.length ?? 0) - 1} onClick={() => moveItem(code, 1)}>↓</button><button type="button" className="ax-btn" aria-label={`${s.removeAria} ${code}`} onClick={() => { patchSelected({ items: (selected.items ?? []).filter(item => item !== code) }); setSelectedItem(""); }}>✕</button></div></li>)}</ol>

  ...
  return <form action={formAction} className={styles.editorForm} aria-labelledby={`designer-${versionId}`}>
    ...
    <div className={styles.designerStudio}>
      ...
      <section className={`${styles.studioPane} ${styles.editorCanvas}`}><h5 style={{ margin: 0 }}>{s.fieldCanvas}</h5>
        {selected ? <>
          <div className="row" style={{ gap: 8 }}><button type="button" className="ax-btn" onClick={() => moveSection(-1)} disabled={selectedIndex === 0}>{s.moveUp}</button><button type="button" className="ax-btn" onClick={() => moveSection(1)} disabled={selectedIndex === sections.length - 1}>{s.moveDown}</button></div>
          ...
          <ol className="stack" style={{ paddingInlineStart: 20 }}>{(selected.items ?? []).map((code, index) => <li key={code}><div className="row" style={{ gap: 6, flexWrap: "wrap" }}><button type="button" className="ax-btn ax-btn--subtle" onClick={() => setSelectedItem(code)}><bdi dir="ltr">{code}</bdi></button><button type="button" className="ax-btn" aria-label={`${s.moveUp} ${code}`} disabled={index === 0} onClick={() => moveItem(code, -1)}>↑</button><button type="button" className="ax-btn" aria-label={`${s.moveDown} ${code}`} disabled={index === (selected.items?.length ?? 0) - 1} onClick={() => moveItem(code, 1)}>↓</button><button type="button" className="ax-btn" aria-label={`${s.removeAria} ${code}`} onClick={() => { patchSelected({ items: (selected.items ?? []).filter(item => item !== code) }); setSelectedItem(""); }}>✕</button></div></li>)}</ol>

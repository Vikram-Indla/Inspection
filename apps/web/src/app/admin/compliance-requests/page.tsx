...
export default async function ComplianceRequestRegister() {
  ...
  return (
    <Shell current="/admin/compliance-requests" title={t("admin.ccr.register.title", "Compliance Configuration Requests")}
      ...
      {error ? (
        <div className="panel"><div className="ax-state" role="alert"><span className="ax-state__glyph" aria-hidden="true">⚠</span><h4>Request register unavailable</h4><p className="t-caption">The read failed. Counts and status have not been inferred.</p><a className="ax-link" href="/admin/compliance-requests">Retry</a></div></div>
      ) : rows.length === 0 ? (
        <div className="panel"><div className="ax-state" role="status"><span className="ax-state__glyph" aria-hidden="true">◇</span><h4>No configuration requests</h4><p className="t-caption">The RLS-scoped read succeeded and returned zero requests. Create the first governed request when authorized.</p>{canCreate ? <Link className="btn btn-secondary btn-touch" href="/admin/compliance-requests/new">Create Request</Link> : null}</div></div>
      ) : (
        <div className="ax-tablewrap"><table className="ax-table"><caption className="sr-only">Compliance Configuration Request register</caption><thead><tr><th scope="col">Request</th><th scope="col">Type</th><th scope="col">Status</th><th scope="col">Revision</th><th scope="col">Created</th><th scope="col">Open</th></tr></thead><tbody>
          {rows.map(row => <tr key={row.id}><th scope="row"><strong>{row.request_number}</strong><div className="t-caption">{row.title}</div></th><td>{row.request_type === "create" ? "Create" : "Modify"}</td><td><span className={`ax-lozenge ccr-status ccr-status--${row.status}`}>{statusLabel[row.status] ?? row.status}</span></td><td className="numeric">R{row.current_revision}</td><td className="numeric">{new Date(row.created_at).toISOString().slice(0, 10)}</td><td><Link className="ax-link" href={`/admin/compliance-requests/${row.id}`}>Open workspace</Link></td></tr>)}
        </tbody></table></div>
      )}
    </Shell>
  );

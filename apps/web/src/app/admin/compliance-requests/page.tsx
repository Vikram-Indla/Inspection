...
export default async function ComplianceRequestRegister() {
  ...
  return (
    <Shell current="/admin/compliance-requests" title={t("admin.ccr.register.title", "Compliance Configuration Requests")}
      context={<><span className="ax-lozenge ax-lozenge--info">CMP-REQ-CCR-001..010</span><span className="t-caption">Governed request register · RLS-scoped</span></>}>
      ...
      {error ? (
        <div className="ax-surface"><div className="ax-state" role="alert"><span className="ax-state__glyph" aria-hidden="true">⚠</span><h4>Request register unavailable</h4><p className="t-caption">The read failed. Counts and status have not been inferred.</p><a className="ax-link" href="/admin/compliance-requests">Retry</a></div></div>
      ) : rows.length === 0 ? (
        <div className="ax-surface"><div className="ax-state" role="status"><span className="ax-state__glyph" aria-hidden="true">◇</span><h4>No configuration requests</h4><p className="t-caption">The RLS-scoped read succeeded and returned zero requests. Create the first governed request when authorized.</p>{canCreate ? <Link className="ax-btn ax-btn--secondary" href="/admin/compliance-requests/new">Create Request</Link> : null}</div></div>
      ) : (

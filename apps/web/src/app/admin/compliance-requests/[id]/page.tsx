...
export default async function ComplianceRequestWorkspace({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  if (readFailed) return (
    <Shell current={shellCurrent} title="Compliance Configuration Request">
      <div className="panel"><div className="ax-state" role="alert"><span className="ax-state__glyph" aria-hidden="true">⚠</span><h4>Request workspace unavailable</h4><p className="t-caption">One or more governed reads failed. No status, revision, component, or decision has been inferred.</p><a className="ax-link" href={`/admin/compliance-requests/${id}`}>Retry</a></div></div>
    </Shell>
  );
  if (!request) return (
    <Shell current={shellCurrent} title="Compliance Configuration Request">
      <div className="panel"><div className="ax-state" role="status"><span className="ax-state__glyph" aria-hidden="true">🔎</span><h4>Request not found or outside your scope</h4><p className="t-caption">The RLS-scoped read returned no request. No configuration content was loaded.</p><Link className="ax-link" href="/admin/compliance-requests">Return to register</Link></div></div>
    </Shell>
  );
    ...
    <Shell current={shellCurrent} title={`${request.request_number} — ${request.title}`}
      ...
      <section className="ccr-grid" aria-label="Request components and dependencies">
        <div className="stack">
          ...
          {currentComponents.length === 0 ? <div className="panel"><div className="ax-state" role="status"><span className="ax-state__glyph" aria-hidden="true">◇</span><h4>No components in this revision</h4><p className="t-caption">Add a Regulation, Inspection Item, Violation, or Penalty component before submission.</p></div></div> : currentComponents.map((component, index) => {
      ...
      <section className="panel ccr-action-zone" aria-labelledby="ccr-actions-heading"><div><h3 id="ccr-actions-heading">Governed actions</h3><p className="t-caption">All transitions are RPC-, role-, RLS-, maker-checker-, audit-, and correlation-enforced.</p></div><div className="ccr-action-list">

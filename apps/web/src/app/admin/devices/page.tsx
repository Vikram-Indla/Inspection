...
export default async function DevicesPage() {
  ...
  return (
    <Shell current="/admin/devices" title={t("mvp3.devices.title", "Trusted device and offline administration")}
      ...
      <section className="ax-surface stack" style={{ padding: "var(--ax-space-300)", marginBlockStart: "var(--ax-space-200)" }}><h3>{t("mvp3.devices.commands", "Command evidence")}</h3>{(commands ?? []).map(row => <div className="row" key={row.id} style={{ justifyContent: "space-between", flexWrap: "wrap" }}><span><strong>{row.command}</strong><small className="ax-caption"> · {row.reason}</small></span><span className="ax-lozenge">{row.status}</span></div>)}{!(commands ?? []).length ? <p className="ax-caption">{t("mvp3.devices.noCommands", "No RLS-visible commands.")}</p> : null}</section>
    </Shell>
  );
}

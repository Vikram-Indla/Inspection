...
export function CreateLayer({ strings: s }: { strings: { key: string; label: string; type: string; create: string; creating: string; created: string } }) {
  ...
  return (
    <form action={action} className="ax-surface" style={{ padding: "var(--ax-space-300)", display: "flex", gap: "var(--ax-space-150)", alignItems: "flex-end", flexWrap: "wrap" }}>
      <div className="ax-field"><label className="ax-field__label">{s.key}</label><input className="ax-input ax-numeric" name="layer_key" required /></div>
      <div className="ax-field"><label className="ax-field__label">{s.label}</label><input className="ax-input" name="label" required /></div>
      <div className="ax-field"><label className="ax-field__label">{s.type}</label>
        <select className="ax-input" name="layer_type"><option>overlay</option><option>base</option><option>boundary</option><option>heat</option><option>route</option></select></div>

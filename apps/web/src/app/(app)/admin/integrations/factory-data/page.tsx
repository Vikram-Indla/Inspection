...
export default async function FactoryDataPage({ searchParams }: { searchParams: Promise<{ factory?: string }> }) {
  ...
  return <Shell current="/admin/integrations" title="Factory data integration" context={<span className="badge badge-info">TASK-FACTORY-360-COMPLETE-010 · P12</span>}>
    <div className="sq-banner"><div><strong>Control plane only.</strong> Factory 360 dossiers are read-only. Provider configuration does not prove connectivity, CSV rows remain staged until a separately governed reconciliation, and source identifiers cannot be silently replaced.</div></div>
    {schemaUnavailable ? <div className="sq-banner sq-banner--warning" role="alert"><div>Factory 360 integration schema is not available in this environment. No provider or history state is inferred.</div></div> : null}
    ...
    <section className="panel stack" style={{ padding: "var(--space-6)" }} aria-labelledby="sync-history"><h3 id="sync-history">Sync and import history</h3>

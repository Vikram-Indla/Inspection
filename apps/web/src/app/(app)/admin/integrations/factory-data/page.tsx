  ...
  return <Shell current="/admin/integrations" title={t("admin.factoryData.title", "Factory data integration")}>
    ...
    <div className="sq-grid" style={{ marginBlock: "var(--space-4)" }}>
      <section className="panel stack" style={{ padding: "var(--space-6)", minWidth: 0 }} aria-labelledby="provider-state"><h3 id="provider-state">{t("admin.factoryData.provider", "Senaei provider")}</h3>
        ...
        <p className="t-caption">{t("admin.factoryData.providerHelp", "The endpoint contract and its recorded live-call check are shown in")} <Link href="/admin/integrations/senai-data?tab=endpoints">{t("admin.factoryData.senaiLink", "SENAI data management")}</Link>. {t("admin.factoryData.configNotConnectivity", "Setting it up is not the same as being connected.")}</p>
      </section>
    ...
    <section className="panel stack" style={{ padding: "var(--space-6)", marginBlockEnd: "var(--space-4)", minWidth: 0 }} aria-labelledby="master-data"><h3 id="master-data">{t("admin.factoryData.masterData", "Factory master-data controls")}</h3>
      <p className="t-caption">{t("admin.factoryData.masterDataHelp", "These operations are always recorded in the Activity Log. They do not edit external CR, license, or plant identifiers.")}</p>
      <p className="t-caption"><strong>{t("admin.factoryData.noWriteback", "Read-only — no write-back.")}</strong> {t("admin.factoryData.noWritebackHelp", "Everything saved here is recorded in SAQEEL only. No establishment or factory master-data change is sent to SENAI: the endpoint contract has no master-data write endpoint, so there is no path for one. Where SENAI stays the source of truth, this screen holds SAQEEL-side details as pending-integration scaffolding until an approved reconciliation runs — it does not change the source.")}</p>
      {factoriesRead.error ? <div className="alert alert-warning" role="alert">{t("admin.factoryData.factoriesError", "The Factory list could not be read. Import and reconciliation history below may still be available.")}</div> : null}
      <form method="get" className="row"><label className="sq-field" style={{ flex: "1 1 16rem", minWidth: 0 }}><span className="sq-field__label">{t("admin.factoryData.factory", "Factory")}</span><select className="select" name="factory" defaultValue={selectedFactory}><option value="">{t("admin.factoryData.selectFactory", "Select a factory")}</option>{factories.map(factory => <option key={factory.id} value={factory.id}>{factory.name} · {factory.factory_code ?? factory.cr_number ?? "—"}</option>)}</select></label><button className="btn btn-secondary btn-touch">{t("admin.factoryData.openControls", "Open controls")}</button></form>
      {selected ? <><p style={{ overflowWrap: "anywhere" }}><strong>{selected.name}</strong> · CR <bdi dir="ltr">{selected.cr_number ?? "—"}</bdi> · {t("admin.factoryData.license", "License")} <bdi dir="ltr">{selected.license_number ?? "—"}</bdi> · <Link href={`/factories/${selected.id}`}>{t("admin.factoryData.openDossier", "Open factory profile (read-only)")}</Link></p>{representativesRead.error ? <div className="alert alert-warning" role="alert">{t("admin.factoryData.representativesError", "Representatives could not be read for this factory. Other factory facts remain available.")}</div> : <MasterDataForms factoryId={selected.id} representatives={representativesRead.data ?? []} />}</> : null}
    </section>
    ...
    <section className="panel stack" style={{ padding: "var(--space-6)", minWidth: 0 }} aria-labelledby="sync-history"><h3 id="sync-history">{t("admin.factoryData.history", "Sync and import history")}</h3>
      {runsRead.error ? <div className="alert alert-warning" role="alert">{t("admin.factoryData.runsError", "Sync runs could not be read. Rejected rows and reconciliation records remain independently readable below.")}</div> : null}
      {batchesRead.error ? <div className="alert alert-warning" role="alert">{t("admin.factoryData.batchesError", "Batch tracking details could not be read. Available sync-run facts are kept without a guessed file origin.")}</div> : null}
      <div className="table-wrap"><table className="table"><thead><tr><th scope="col">{t("admin.factoryData.created", "Created")}</th><th scope="col">{t("admin.factoryData.modeSource", "Mode / source")}</th><th scope="col">{t("common.status", "Status")}</th><th scope="col">{t("admin.factoryData.custody", "Custody")}</th><th scope="col">{t("admin.factoryData.counts", "Received / accepted / rejected")}</th></tr></thead><tbody>{(runsRead.data ?? []).map(run => { const batch = batchByRun.get(run.id); return <tr key={run.id}><td><time dateTime={run.created_at}>{new Date(run.created_at).toLocaleString()}</time></td><td>{statusLabel(run.mode)}<div className="t-caption"><bdi dir="ltr">{run.mode}</bdi> · {run.source_file_name ?? t("admin.factoryData.providerSource", "provider")}</div></td><td><span className="badge">{statusLabel(run.status)}</span><div className="t-caption"><bdi dir="ltr">{run.status}</bdi></div>{batch ? <div className="t-caption">{t("admin.factoryData.batch", "batch")} {statusLabel(batch.status)} · <bdi dir="ltr">{batch.status}</bdi></div> : null}</td><td className="t-caption" style={{ overflowWrap: "anywhere" }}><bdi dir="ltr">{run.correlation_id}</bdi>{batch ? <div>SHA-256 <bdi dir="ltr">{batch.file_sha256.slice(0, 12)}…</bdi></div> : <div>{t("admin.factoryData.noBatchProvenance", "No batch provenance recorded")}</div>}</td><td className="numeric">{run.rows_received} / {run.rows_accepted} / {run.rows_rejected}</td></tr>})}{!runsRead.error && !(runsRead.data ?? []).length ? <tr><td colSpan={5}>{t("admin.factoryData.noRuns", "No sync or import runs visible to you. This is a checked, empty result.")}</td></tr> : null}</tbody></table></div>
    </section>

    <div className="sq-grid" style={{ marginBlockStart: "var(--space-4)" }}>
      <section className="panel stack" style={{ padding: "var(--space-6)", minWidth: 0 }}><h3>{t("admin.factoryData.rejected", "Rejected staged rows")}</h3>{rowsRead.error ? <div className="alert alert-warning" role="alert">{t("admin.factoryData.rowsError", "Rejected staged rows could not be read. This is data that's not available, not an empty rejection set.")}</div> : null}{(rowsRead.data ?? []).map(row => <div key={row.id} style={{ overflowWrap: "anywhere" }}><strong>{t("admin.factoryData.row", "Row")} {row.row_number}</strong> <span className="badge badge-critical">{statusLabel(row.status)}</span><div className="t-caption"><bdi dir="ltr">{row.status}</bdi></div><p className="t-caption"><bdi dir="ltr">{row.safe_error_codes.join(", ")}</bdi></p></div>)}{!rowsRead.error && !(rowsRead.data ?? []).length ? <p className="t-caption">{t("admin.factoryData.noRejected", "No rejected staged rows. This is a checked, empty result.")}</p> : null}</section>
      <section className="panel stack" style={{ padding: "var(--space-6)", minWidth: 0 }}><h3>{t("admin.factoryData.reconciliation", "Reconciliation history")}</h3>{reconciliationRead.error ? <div className="alert alert-warning" role="alert">{t("admin.factoryData.reconciliationError", "Reconciliation history could not be read. This is data that's not available, not an empty history.")}</div> : null}{(reconciliationRead.data ?? []).map(row => <div key={row.id} style={{ overflowWrap: "anywhere" }}><strong>{statusLabel(row.entity_type)}</strong><div className="t-caption"><bdi dir="ltr">{row.entity_type}</bdi> · <bdi dir="ltr">{row.external_id ?? "—"}</bdi></div><span className="badge">{statusLabel(row.outcome)}</span><div className="t-caption"><bdi dir="ltr">{row.outcome}</bdi> · <time dateTime={row.reconciled_at}>{new Date(row.reconciled_at).toLocaleString()}</time></div><p className="t-caption"><bdi dir="ltr">{row.safe_reason_codes.join(", ") || t("admin.factoryData.noReason", "No reason code")}</bdi></p></div>)}{!reconciliationRead.error && !(reconciliationRead.data ?? []).length ? <p className="t-caption">{t("admin.factoryData.noReconciliation", "No reconciliation records visible to you. This is a checked, empty result. Staged does not mean imported.")}</p> : null}</section>
    </div>
  </Shell>;
}

...
export default async function FactoryDataPage({ searchParams }: { searchParams: Promise<{ factory?: string }> }) {
  ...
  return <Shell current="/admin/integrations" title={t("admin.factoryData.title", "Factory data integration")} context={<span className="badge badge-info">TASK-FACTORY-360-COMPLETE-010 · P12</span>}>
    <div className="alert"><div><strong>{t("admin.factoryData.controlPlane", "Control plane only.")}</strong> {t("admin.factoryData.truth", "Factory 360 profiles are read-only. Provider configuration does not prove connectivity, CSV rows remain staged until a separately governed reconciliation, and source identifiers cannot be silently replaced.")}</div></div>
    ...
    <section className="panel stack" style={{ padding: "var(--space-6)", marginBlockEnd: "var(--space-4)", minWidth: 0 }} aria-labelledby="master-data"><h3 id="master-data">{t("admin.factoryData.masterData", "Governed factory master-data controls")}</h3>
      ...
      {factoriesRead.error ? <div className="alert alert-warning" role="alert">{t("admin.factoryData.factoriesError", "The factory registry could not be read. Import and reconciliation history below may still be available.")}</div> : null}
      ...
      {selected ? <><p style={{ overflowWrap: "anywhere" }}><strong>{selected.name}</strong> · CR <bdi dir="ltr">{selected.cr_number ?? "—"}</bdi> · {t("admin.factoryData.license", "License")} <bdi dir="ltr">{selected.license_number ?? "—"}</bdi> · <Link href={`/factories/${selected.id}`}>{t("admin.factoryData.openDossier", "Open read-only dossier")}</Link></p>{representativesRead.error ? <div className="alert alert-warning" role="alert">{t("admin.factoryData.representativesError", "Representatives could not be read for this factory. Other factory facts remain available.")}</div> : <MasterDataForms factoryId={selected.id} representatives={representativesRead.data ?? []} />}</> : null}
    </section>
    ...
    <section className="panel stack" style={{ padding: "var(--space-6)", minWidth: 0 }} aria-labelledby="sync-history"><h3 id="sync-history">{t("admin.factoryData.history", "Sync and import history")}</h3>
      ...
      {batchesRead.error ? <div className="alert alert-warning" role="alert">{t("admin.factoryData.batchesError", "Batch custody metadata could not be read. Available sync-run facts are preserved without inferred file provenance.")}</div> : null}

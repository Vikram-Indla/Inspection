...
export default async function PlatformOperationsPage() {
  ...
  return (
    <Shell current="/admin/operations" title={t("mvp3.operations.title", "Platform operations and resilience")}
      context={<span className="badge badge-info">{"M3-00 · CD-050 · "}{t("mvp3.operations.badge", "fail-closed operations")}</span>}>
      {sourceFailures.length ? <div className="sq-banner sq-banner--warning" role="alert"><div><strong>{t("mvp3.operations.sourcesUnavailable", "Some operational sources are unavailable.")}</strong>{" "}{t("mvp3.operations.sourcesUnavailableBody", "Unavailable sources are identified below and are never represented as zero or empty.")}<span className="t-caption" style={{ display: "block", marginBlockStart: "var(--space-2)" }}>{sourceFailures.join(" · ")}</span></div></div> : null}
      <div className="sq-grid">
        <section className="panel" style={{ padding: "var(--space-6)" }}><p className="t-caption">{t("mvp3.operations.endpoints", "Endpoint contracts")}</p><strong className="sq-display">{endpointsError ? t("common.unavailable", "Unavailable") : endpointRows.length}</strong>{!endpointsError ? <p>{endpointRows.filter(x => x.status === "configured").length} {t("mvp3.operations.configured", "configured")}</p> : null}</section>
        <section className="panel" style={{ padding: "var(--space-6)" }}><p className="t-caption">{t("mvp3.operations.openErrors", "Open error records")}</p><strong className="sq-display">{errorsError ? t("common.unavailable", "Unavailable") : (errors ?? []).filter(x => !["resolved"].includes(x.status)).length}</strong>{!errorsError ? <p>{t("mvp3.operations.noThroughput", "No throughput claim is derived from this count.")}</p> : null}</section>
        <section className="panel" style={{ padding: "var(--space-6)" }}><p className="t-caption">{t("mvp3.operations.publishedFlags", "Published flag versions")}</p><strong className="sq-display">{flagsError ? t("common.unavailable", "Unavailable") : (flags ?? []).filter(x => x.status === "published").length}</strong>{!flagsError ? <p>{t("mvp3.operations.sod", "Maker-checker enforced.")}</p> : null}</section>
      </div>
      <section className="panel stack" style={{ padding: "var(--space-6)", marginBlockStart: "var(--space-4)" }}><h3>{t("mvp3.operations.errors", "Error queue")}</h3>{errorsError ? <div className="sq-banner sq-banner--warning" role="status">{t("mvp3.operations.sourceUnavailable", "This source is unavailable. No empty-state claim is made.")}</div> : <div className="sq-tablewrap"><table className="sq-table"><thead><tr><th scope="col">{t("common.source", "Source")}</th><th scope="col">{t("common.operation", "Operation")}</th><th scope="col">{t("common.status", "Status")}</th><th scope="col">{t("mvp3.operations.attempts", "Attempts")}</th><th scope="col">{t("common.action", "Governed action")}</th></tr></thead><tbody>{(errors ?? []).map(row => <tr key={row.id}><th scope="row">{row.source}</th><td>{row.operation}</td><td><span className="badge badge-warning">{row.status}</span><div className="t-caption">{row.last_error_code ?? "—"}</div></td><td className="numeric">{row.attempt_count}</td><td>{["failed","dependency_blocked"].includes(row.status) ? <Mvp3ActionForm action={requestErrorRetry} submitLabel={t("mvp3.operations.retry", "Request idempotent retry")}><input type="hidden" name="errorId" value={row.id}/></Mvp3ActionForm> : "—"}</td></tr>)}{!(errors ?? []).length ? <tr><td colSpan={5}>{t("mvp3.operations.noErrors", "No RLS-visible error records. This is not a platform-health assertion.")}</td></tr> : null}</tbody></table></div>}</section>
      <section className="panel stack" style={{ padding: "var(--space-6)", marginBlockStart: "var(--space-4)" }}><h3>{t("mvp3.operations.flags", "Feature flag versions")}</h3>{flagsError ? <div className="sq-banner sq-banner--warning" role="status">{t("mvp3.operations.sourceUnavailable", "This source is unavailable. No empty-state claim is made.")}</div> : <div className="sq-tablewrap"><table className="sq-table"><thead><tr><th scope="col">{t("common.name", "Flag")}</th><th scope="col">{t("common.version", "Version")}</th><th scope="col">{t("common.status", "State")}</th><th scope="col">{t("mvp3.operations.value", "Value")}</th><th scope="col">{t("common.action", "Checker action")}</th></tr></thead><tbody>{(flags ?? []).map(row => <tr key={row.id}><th scope="row">{row.flag_key}<div className="t-caption">{row.description}</div></th><td>{row.version}</td><td><span className="badge">{row.status}</span></td><td>{row.enabled ? t("common.enabled", "Enabled") : t("common.disabled", "Disabled")}</td><td>{row.status === "draft" ? <Mvp3ActionForm action={publishFeatureFlag} submitLabel={t("mvp3.operations.publish", "Approve and publish")}><input type="hidden" name="flagId" value={row.id}/></Mvp3ActionForm> : "—"}</td></tr>)}{!(flags ?? []).length ? <tr><td colSpan={5}>{t("mvp3.operations.noFlags", "No feature flag versions.")}</td></tr> : null}</tbody></table></div>}</section>
      <div className="sq-banner sq-banner--warning" style={{ marginBlockStart: "var(--space-4)" }}><div><strong>{t("mvp3.operations.policyHold", "Retention, backup target and restore objectives remain policy-held.")}</strong> {t("mvp3.operations.policyHoldBody", "No purge or successful restore claim is enabled until approved values and executed evidence exist.")}</div></div>
    </Shell>
  );
}

  ...
  return (
    <Shell current="/admin/operations" title={t("mvp3.operations.title", "Platform operations and resilience")}
      ...
      <div className="ax-grid">
        <section className="panel" style={{ padding: "var(--space-6)" }}><p className="t-caption">{t("mvp3.operations.endpoints", "Endpoint contracts")}</p><strong className="ax-display">{endpointRows.length}</strong><p>{endpointRows.filter(x => x.status === "configured").length} {t("mvp3.operations.configured", "configured")}</p></section>
        <section className="panel" style={{ padding: "var(--space-6)" }}><p className="t-caption">{t("mvp3.operations.openErrors", "Open error records")}</p><strong className="ax-display">{(errors ?? []).filter(x => !["resolved"].includes(x.status)).length}</strong><p>{t("mvp3.operations.noThroughput", "No throughput claim is derived from this count.")}</p></section>
        <section className="panel" style={{ padding: "var(--space-6)" }}><p className="t-caption">{t("mvp3.operations.publishedFlags", "Published flag versions")}</p><strong className="ax-display">{(flags ?? []).filter(x => x.status === "published").length}</strong><p>{t("mvp3.operations.sod", "Maker-checker enforced.")}</p></section>
      </div>
      <section className="panel stack" style={{ padding: "var(--space-6)", marginBlockStart: "var(--space-4)" }}><h3>{t("mvp3.operations.errors", "Error queue")}</h3><div className="ax-tablewrap"><table className="ax-table"><thead><tr><th scope="col">{t("common.source", "Source")}</th><th scope="col">{t("common.operation", "Operation")}</th><th scope="col">{t("common.status", "Status")}</th><th scope="col">{t("mvp3.operations.attempts", "Attempts")}</th><th scope="col">{t("common.action", "Governed action")}</th></tr></thead><tbody>{(errors ?? []).map(row => <tr key={row.id}><th scope="row">{row.source}</th><td>{row.operation}</td><td><span className="badge badge-warning">{row.status}</span><div className="t-caption">{row.last_error_code ?? "—"}</div></td><td className="numeric">{row.attempt_count}</td><td>{["failed","dependency_blocked"].includes(row.status) ? <Mvp3ActionForm action={requestErrorRetry} submitLabel={t("mvp3.operations.retry", "Request idempotent retry")}><input type="hidden" name="errorId" value={row.id}/></Mvp3ActionForm> : "—"}</td></tr>)}{!error && !(errors ?? []).length ? <tr><td colSpan={5}>{t("mvp3.operations.noErrors", "No RLS-visible error records. This is not a platform-health assertion.")}</td></tr> : null}</tbody></table></div></section>

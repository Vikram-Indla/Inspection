...
export function CreateLayer({ strings: s }: { strings: { key: string; label: string; type: string; create: string; creating: string; created: string } }) {
  ...
  return (
    <form action={action} className="panel" style={{ padding: "var(--space-6)", display: "flex", gap: "var(--space-3)", alignItems: "flex-end", flexWrap: "wrap" }}>
      <div className="sq-field"><label className="sq-field__label" htmlFor="gis-layer-key">{s.key}</label><input className="sq-input numeric" name="layer_key" id="gis-layer-key" required /></div>
      <div className="sq-field"><label className="sq-field__label" htmlFor="gis-layer-label">{s.label}</label><input className="sq-input" name="label" id="gis-layer-label" required /></div>
      <div className="sq-field"><label className="sq-field__label" htmlFor="gis-layer-type">{s.type}</label>
        <select className="sq-input" name="layer_type" id="gis-layer-type"><option>overlay</option><option>base</option><option>boundary</option><option>heat</option><option>route</option></select></div>

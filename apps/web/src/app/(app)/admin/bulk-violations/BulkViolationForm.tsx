  ...
  return (
    <form action={formAction} className="stack" style={{ gap: "var(--space-6)" }}>
      ...
      <section className="panel stack" style={{ padding: "var(--space-6)" }}>
        ...
        <label className="ax-field" style={{ maxInlineSize: "none" }}>
          <span className="ax-field__label">{strings.notesLabel}</span>
          <textarea className="ax-textarea" name="notes" rows={2} value={notes} onChange={e => setNotes(e.target.value)} placeholder={strings.notesPlaceholder} />
        </label>
      </section>
      ...
      {results.length > 0 && (
        <section className="panel stack" style={{ padding: "var(--space-6)" }}>
          ...
          {failedCount > 0
            ...
            : <div className="ax-banner ax-banner--success"><div>{fmt(strings.allSucceeded, { n: successCount })}</div></div>}

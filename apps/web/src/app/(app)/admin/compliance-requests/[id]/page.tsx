...
export default async function ComplianceRequestWorkspace({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  if (readFailed) return (
    <Shell current={shellCurrent} title="Compliance Configuration Request">
      <div className="panel"><div className="sq-state" role="alert"><span className="sq-state__glyph" aria-hidden="true">⚠</span><h4>Request workspace unavailable</h4><p className="t-caption">One or more governed reads failed. No status, revision, component, or decision has been inferred.</p><a className="sq-link" href={`/admin/compliance-requests/${id}`}>Retry</a></div></div>
    </Shell>
  );
  if (!request) return (
    <Shell current={shellCurrent} title="Compliance Configuration Request">
      <div className="panel"><div className="sq-state" role="status"><span className="sq-state__glyph" aria-hidden="true">🔎</span><h4>Request not found or outside your scope</h4><p className="t-caption">The RLS-scoped read returned no request. No configuration content was loaded.</p><Link className="sq-link" href="/admin/compliance-requests">Return to register</Link></div></div>
    </Shell>
  );
  ...
  return (
    <Shell current={shellCurrent} title={`${request.request_number} — ${request.title}`}
      context={<><span className={`sq-lozenge ccr-status ccr-status--${request.status}`}>{label(request.status)}</span><span className="t-caption">Revision {request.current_revision} · Correlation <bdi>{request.correlation_id}</bdi></span></>}>
      <p className="t-caption"><Link className="sq-link" href={fromQueue ? "/admin/compliance-approvals" : "/admin/compliance-requests"}>← {fromQueue ? "Compliance Approval Queue" : "Request register"}</Link></p>

      {request.return_reason ? <div className="sq-banner sq-banner--warning" role="alert"><strong>Return or rejection reason:</strong> {request.return_reason}</div> : null}
      {!isOwner && !canReview ? <div className="sq-banner" role="note"><strong>Read-only.</strong> Your role can inspect this governed request but cannot change or decide it.</div> : null}
      ...
      {editable ? <section className="panel ccr-form-card" aria-labelledby="ccr-draft-heading"><h3 id="ccr-draft-heading">Draft details</h3><ActionForm action={updateComplianceRequestDraft} className="ccr-form"><input type="hidden" name="request_id" value={request.id}/><label className="sq-field"><span className="sq-field__label">Title</span><input className="sq-input" name="title" required defaultValue={request.title}/></label><label className="sq-field ccr-span"><span className="sq-field__label">Description</span><textarea className="sq-textarea" name="description" rows={3} defaultValue={request.description ?? ""}/></label><label className="sq-field ccr-span"><span className="sq-field__label">Comments</span><textarea className="sq-textarea" name="comments" rows={2} defaultValue={request.comments ?? ""}/></label><div className="ccr-span"><button className="btn btn-secondary btn-touch">Save Draft</button></div></ActionForm></section> : null}
      ...
      <section className="ccr-grid" aria-label="Request components and dependencies">
        <div className="stack">
          ...
          {currentComponents.length === 0 ? <div className="panel"><div className="sq-state" role="status"><span className="sq-state__glyph" aria-hidden="true">◇</span><h4>No components in this revision</h4><p className="t-caption">Add a Regulation, Inspection Item, Violation, or Penalty component before submission.</p></div></div> : currentComponents.map((component, index) => {
      ...
      {editable ? <section className="panel ccr-form-card" aria-labelledby="ccr-add-heading"><h3 id="ccr-add-heading">Add component</h3><p className="t-caption">Snapshots are JSON objects. For a child-only modification, include its existing parent version key: <code>regulation_version_id</code>, <code>inspection_item_version_id</code>, or <code>violation_version_id</code>.</p><ActionForm action={addComplianceRequestComponent} className="ccr-form"><input type="hidden" name="request_id" value={request.id}/><label className="sq-field"><span className="sq-field__label">Component type</span><select className="sq-select" name="entity_kind" required><option value="regulation">Regulation</option><option value="inspection_item">Inspection Item</option><option value="violation">Violation</option><option value="penalty">Penalty</option></select></label><label className="sq-field"><span className="sq-field__label">Existing target UUID</span><input className="sq-input" name="target_entity_id" placeholder={request.request_type === "modify" ? "Required for Modify" : "Optional"}/></label><label className="sq-field"><span className="sq-field__label">Current-value snapshot</span><textarea className="sq-textarea ccr-json" name="current_snapshot" rows={8} defaultValue="{}"/></label><label className="sq-field"><span className="sq-field__label">Proposed-value snapshot</span><textarea className="sq-textarea ccr-json" name="proposed_snapshot" rows={8} required defaultValue={'{\n  "code": "",\n  "title": ""\n}'}/></label><label className="sq-field ccr-span"><span className="sq-field__label">Component comments</span><textarea className="sq-textarea" name="component_comments" rows={2}/></label><div className="ccr-span"><button className="btn btn-secondary btn-touch">Add component</button></div></ActionForm></section> : null}
      ...
      <section className="panel ccr-action-zone" aria-labelledby="ccr-actions-heading"><div><h3 id="ccr-actions-heading">Governed actions</h3><p className="t-caption">All transitions are RPC-, role-, RLS-, maker-checker-, audit-, and correlation-enforced.</p></div><div className="ccr-action-list">
        ...
        {isOwner && canWrite && request.status === "returned" ? <ActionForm action={reviseComplianceRequest} className="ccr-inline-form"><input type="hidden" name="request_id" value={request.id}/><input className="sq-input" name="comments" placeholder="Revision summary"/><button className="btn btn-primary btn-lg btn-touch">Create revision {request.current_revision + 1}</button></ActionForm> : null}
        {reviewable ? <><ActionForm action={returnComplianceRequest} className="ccr-inline-form"><input type="hidden" name="request_id" value={request.id}/><input className="sq-input" name="comments" required placeholder="Return reason (required)"/><button className="btn btn-secondary btn-touch">Return request</button></ActionForm><ActionForm action={rejectComplianceRequest} className="ccr-inline-form"><input type="hidden" name="request_id" value={request.id}/><input className="sq-input" name="comments" required placeholder="Rejection reason (required)"/><button className="btn btn-danger btn-touch">Reject request</button></ActionForm></> : null}

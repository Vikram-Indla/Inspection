...
export default async function ComplianceRequestWorkspace({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  if (readFailed) return (
    <Shell current={shellCurrent} title="Compliance Configuration Request">
      <div className="panel"><div className="saqeel-state" role="alert"><span className="saqeel-state__glyph" aria-hidden="true">⚠</span><h4>Request workspace unavailable</h4><p className="t-caption">One or more governed reads failed. No status, revision, component, or decision has been inferred.</p><a className="sq-link" href={`/admin/compliance-requests/${id}`}>Retry</a></div></div>
    </Shell>
  );
  if (!request) return (
    <Shell current={shellCurrent} title="Compliance Configuration Request">
      <div className="panel"><div className="saqeel-state" role="status"><span className="saqeel-state__glyph" aria-hidden="true">🔎</span><h4>Request not found or outside your scope</h4><p className="t-caption">The RLS-scoped read returned no request. No configuration content was loaded.</p><Link className="sq-link" href="/admin/compliance-requests">Return to register</Link></div></div>
    </Shell>
  );
  ...
  return (
    <Shell current={shellCurrent} title={`${request.request_number} — ${request.title}`}
      ...
      {!isOwner && !canReview ? <div className="alert" role="note"><strong>Read-only.</strong> Your role can inspect this governed request but cannot change or decide it.</div> : null}

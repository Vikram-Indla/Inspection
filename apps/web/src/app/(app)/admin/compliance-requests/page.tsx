...
export default async function ComplianceRequestRegister() {
  ...
  return (
    <Shell current="/admin/compliance-requests" title={t("admin.ccr.register.title", "Compliance Configuration Requests")}
      context={<><span className="ax-lozenge ax-lozenge--info">CMP-REQ-CCR-001..010</span><span className="ax-caption">Governed request list · Filtered to your access</span></>}>
      ...
      {error ? (
        ...
        <div className="ax-surface"><div className="ax-state" role="status"><span className="ax-state__glyph" aria-hidden="true">◇</span><h4>No configuration requests</h4><p className="ax-caption">The read succeeded and returned zero requests, filtered to your access. Create the first governed request when authorized.</p>{canCreate ? <Link className="ax-btn ax-btn--secondary" href="/admin/compliance-requests/new">Create Request</Link> : null}</div></div>
      ) : (

...
export default async function RiskStudio() {
  ...
  const labels: RiskLabels = {
    ...
    confirmLive: t("admin.risk.confirmLive", "I understand this setting takes effect right away for new score calculations."),
  ...
  return (
    <AdminDestinationFrame
      ...
      tabs={[
        ...
        { label: t("admin.risk.nav.models", copy("Model versions", "إصدارات النماذج")), href: "/admin/risk/models" },
    ...
    >
      ...
      {error && (
        <div className="alert alert-critical" role="alert"><div>
          <strong>{t("admin.risk.error.title", "Couldn’t load risk settings.")}</strong>{" "}
          {t("admin.risk.error.body", "We couldn’t verify the existing settings. Nothing changed; retry or check your risk-owner access.")}
        </div></div>
      )}

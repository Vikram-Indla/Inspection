...
export default async function ComplianceApprovalQueue({
  ...
  const copy = ar ? {
    ...
    emptyBody: "لا توجد إسنادات مؤهلة لفصل المُنشئ عن المُراجع في هذا العرض.",
    ...
    emptyBody: "No eligible maker-checker assignments for this view.",
  ...
  return (
    <Shell current="/admin/compliance-approvals" title={copy.title} context={<><span className="badge badge-info">{copy.makerChecker}</span><Text as="span" tone="muted">{copy.distinct}</Text></>}>
      ...
      <div className="ccr-register-head"><div><h3>{copy.workload}</h3><Text tone="muted">{copy.workloadHelp}</Text></div><Link className="btn btn-secondary btn-touch" href="/admin/compliance-requests">{copy.register}</Link></div>
      ...
      {queueReadFailed ? <div className="panel"><div className="saqeel-state" role="alert"><span className="saqeel-state__glyph" aria-hidden="true">⚠</span><h4>{copy.unavailableTitle}</h4><Text tone="muted">{copy.unavailableBody}</Text><a className="sq-link" href={`/admin/compliance-approvals?view=${filterKey}`}>{copy.retryQueue}</a></div></div> : rows.length === 0 ? <div className="panel"><div className="saqeel-state" role="status"><span className="saqeel-state__glyph" aria-hidden="true">✓</span><h4>{copy.noPrefix} {filterLabel.toLocaleLowerCase(ar ? "ar" : "en")} {copy.noSuffix}</h4><Text tone="muted">{copy.emptyBody}</Text></div></div> : <div className="cmp-approval-list">{rows.map(row => {
        ...
        return <article className="panel cmp-approval-card" key={row.id}><header><div><Overline><bdi>{row.request_number}</bdi> · {row.request_type === "create" ? copy.create : copy.modify} · {copy.revision} {row.current_revision}</Overline><h3>{row.title}</h3></div><span className={`badge ccr-status ccr-status--${row.status}`}>{statusLabel(row.status, ar)}</span></header><dl className="cmp-approval-facts"><div><dt>{copy.submitted}</dt><dd className="numeric">{timestamp(row.submitted_at, copy.unavailable)}</dd></div><div><dt>{copy.components}</dt><dd>{componentsAvailable ? current.length : copy.unavailable}</dd></div><div><dt>{copy.progress}</dt><dd>{componentsAvailable ? progress || copy.noComponents : copy.unavailable}</dd></div><div><dt>{copy.dependencies}</dt><dd>{dependenciesAvailable ? dependencyCount : copy.unavailable}</dd></div><div><dt>{copy.coverage}</dt><dd>{componentsAvailable ? coverage || copy.noComponents : copy.unavailable}</dd></div><div><dt>{copy.auditReceipts}</dt><dd>{auditCount ?? copy.unavailable}{row.publication_audit_reference ? <> · {copy.publicationReceipt} <bdi>{row.publication_audit_reference}</bdi></> : null}</dd></div></dl><footer><Text as="span" tone="muted">{copy.correlation} <bdi>{row.correlation_id}</bdi> · {copy.immutableRevision}</Text><Link className="btn btn-primary btn-lg btn-touch" href={`/admin/compliance-requests/${row.id}?from=approval-queue`}>{copy.open}</Link></footer></article>;
      })}</div>}
    </Shell>
  );
}

...
export default async function DevicesPage() {
  ...
  const deviceCountLabel = devicesError
    ? t("mvp3.devices.registerUnavailable", "Register unavailable")
  ...
  return (
    <Shell current="/admin/devices" title={t("mvp3.devices.title", "Trusted device and offline administration")}
      context={<span className="badge badge-info">{"M3-06 · CD-056 · "}{deviceCountLabel}</span>}>
      <div className="sq-banner"><div><strong>{t("mvp3.devices.rule", "Only trusted enrolled devices may open official inspection packages.")}</strong> {t("mvp3.devices.ruleBody", "A queued command is not a completed wipe; device acknowledgement remains a separate fact.")}</div></div>
      {devicesError ? <div className="sq-banner sq-banner--warning" role="alert">{t("mvp3.schema.pending", "MVP3 database contract is not applied in this environment. No data is inferred.")}</div> : null}
      <section className="panel stack" style={{ padding: "var(--space-6)", marginBlockStart: "var(--space-4)" }}><div className="row" style={{ justifyContent: "space-between" }}><h3>{t("mvp3.devices.register", "Device trust register")}</h3><span className="badge">{devicesError ? t("common.unavailable", "Unavailable") : `${(devices ?? []).filter(x => x.trust_status === "trusted").length} ${t("mvp3.devices.trusted", "trusted")}`}</span></div><div className="sq-tablewrap"><table className="sq-table"><thead><tr><th scope="col">{t("mvp3.devices.device", "Device")}</th><th scope="col">{t("mvp3.devices.assignee", "Assigned inspector")}</th><th scope="col">{t("common.status", "Trust state")}</th><th scope="col">{t("mvp3.devices.lastSeen", "Last seen")}</th><th scope="col">{t("common.action", "Governed command")}</th></tr></thead><tbody>{(devices ?? []).map(row => <tr key={row.id}><th scope="row">{row.device_identifier}<div className="t-caption">{row.platform} · MDM {row.mdm_reference ?? "unverified"}</div></th><td><bdi>{row.assigned_user_id ?? "unassigned"}</bdi></td><td><span className={`sq-lozenge ${row.trust_status === "trusted" ? "sq-lozenge--success" : "sq-lozenge--warning"}`}>{row.trust_status.replaceAll("_", " ")}</span></td><td>{row.last_seen_at ? new Date(row.last_seen_at).toLocaleString() : t("common.never", "Never")}</td><td><Mvp3ActionForm action={issueDeviceCommand} submitLabel={t("mvp3.devices.queue", "Queue command")}><input type="hidden" name="deviceId" value={row.id}/><label>{t("mvp3.devices.command", "Command")}<select name="command" required defaultValue=""><option value="" disabled>—</option><option value="suspend">{t("mvp3.devices.suspend", "Suspend")}</option><option value="resume">{t("mvp3.devices.resume", "Resume")}</option><option value="expire_packages">{t("mvp3.devices.expire", "Expire packages")}</option><option value="remote_wipe">{t("mvp3.devices.wipe", "Remote wipe")}</option></select></label><label>{t("common.reason", "Reason")}<textarea name="reason" minLength={8} required /></label></Mvp3ActionForm></td></tr>)}{!devicesError && !(devices ?? []).length ? <tr><td colSpan={5}>{t("mvp3.devices.empty", "No RLS-visible enrolled devices.")}</td></tr> : null}</tbody></table></div></section>
      <section className="panel stack" style={{ padding: "var(--space-6)", marginBlockStart: "var(--space-4)" }}><h3>{t("mvp3.devices.commands", "Command evidence")}</h3>{commandsError ? <div className="sq-banner sq-banner--warning" role="alert">{t("mvp3.devices.commandsUnavailable", "Command evidence is unavailable. No command state is inferred.")}</div> : null}{(commands ?? []).map(row => <div className="row" key={row.id} style={{ justifyContent: "space-between", flexWrap: "wrap" }}><span><strong>{row.command}</strong><small className="t-caption"> · {row.reason}</small></span><span className="badge">{row.status}</span></div>)}{!commandsError && !(commands ?? []).length ? <p className="t-caption">{t("mvp3.devices.noCommands", "No RLS-visible commands.")}</p> : null}</section>
    </Shell>
  );
}

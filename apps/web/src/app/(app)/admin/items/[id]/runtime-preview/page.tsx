...
export default async function InspectorRuntimePreview({
  ...
  if (itemError || !item) {
    return <Shell current="/admin/items" title="Inspector Runtime Preview"><div className="panel"><div className="sq-state" role="alert"><span className="sq-state__glyph" aria-hidden="true">🔎</span><h4>{itemError ? "Preview unavailable" : "Inspection item not found"}</h4><p className="t-caption">{itemError ? "The configuration read failed. Retry without assuming the item is absent." : "The read succeeded, but no item has this identifier."}</p><a className="sq-link" href="/admin/items">Back to Compliance Library</a></div></div></Shell>;
  }
  ...
  return (
    <Shell current="/admin/items" title="Inspector Runtime Preview" context={<span className="badge badge-info">Read-only · configuration verification</span>}>
      ...
      <div className="sq-banner" role="note"><strong>Control-plane preview only.</strong> This page reads approved Compliance configuration and immutable package snapshots. It does not author, publish, or change the Inspector application.</div>
      {useError ? <div className="sq-banner sq-banner--warning" role="alert"><strong>Package usage unavailable.</strong> Exact runtime placement is unknown; the item itself is shown from the live library.</div> : null}

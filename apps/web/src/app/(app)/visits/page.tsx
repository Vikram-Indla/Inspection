    ...
    <Shell current={shellCurrent} title={t("visit.list.title", "Visit management")}
      ...
      <div className="ax-row" style={{ justifyContent: "space-between", flexWrap: "wrap", alignItems: "center", gap: "var(--ax-space-150)" }}>
        <div className="ax-row" role="group" aria-label={t("visit.views.aria", "Visit management views")}>
          <Link className="ax-btn ax-btn--secondary" aria-current="page" href={targetPreview ? `${routeBase}?wa_preview=1` : routeBase} prefetch={false}>{t("visit.views.list", "List")}</Link>
        ...
        <Link className="ax-btn ax-btn--subtle" href="/planning" prefetch={false}>
        ...
        <span className={targetPreview ? "ax-numeric" : "ax-caption ax-numeric"}>{t("visit.list.scope", "RLS-scoped — showing {shown} of {total}").replace("{shown}", String(Math.min(rows.length, limit))).replace("{total}", String(total))}</span>
      </div>

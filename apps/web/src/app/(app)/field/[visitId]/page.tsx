...
export default async function FieldVisit({ params, searchParams }: { params: Promise<{ visitId: string }>; searchParams: Promise<{ created?: string }> }) {
  ...
  // No mutation on read: decide_geo_override is the sole race-safe expiry
  // authority. A past-due pending row is only relabeled in memory for display;
  // its stored status is never rewritten by a page view. The previous
  // expire_stale_geo_override_requests RPC here was a security-definer UPDATE
  // over every past-due row in the table, so one inspector opening a visit
  // decided other people's pending requests and stamped decided_at on them.
  ...
  // milliseconds so the relabel is correct regardless of timestamp precision or
  // format. An unparseable or missing expires_at fails Number.isFinite and the
  // row is left exactly as stored (pending), never falsely shown as expired.

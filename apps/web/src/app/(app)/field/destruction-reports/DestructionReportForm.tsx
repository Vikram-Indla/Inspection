...
import { createFieldDestructionReport, type FieldDestructionResult } from "./actions";
import styles from "../incident-reports/incident-reports.module.css";

export type FieldDestructionStrings = {
  reportNumber: string; reportDate: string;
...
// node 362:39096 ("محضر إتلاف منتجات مخالفة"). Same signature-reuse pattern as
// field/summons-notices and field/sample-collection-reports.
export default function FieldDestructionReportForm({ locale, strings: s, sigStrings, context, contextBadge }: {
  locale: string; strings: FieldDestructionStrings; sigStrings: SignaturePadStrings;
  context?: { factoryId?: string; visitId?: string; inspectionId?: string };
  contextBadge?: string;
}) {
  const [state, action, pending] = useActionState<FieldDestructionResult, FormData>(createFieldDestructionReport, {});
  const [signing, setSigning] = useState(false);
  const [ack, setAck] = useState<(SignatureAck & { attendance: "present" | "absent" | "objected" }) | null>(null);

  return (
    <form id="new-destruction-report" action={action} className={styles.card}>
      ...
      <div className={styles.grid2}>
        <div className="field">
          <label htmlFor="dr-report-number">{s.reportNumber}<span className={styles.req} aria-hidden="true"> *</span></label>
          <input className="input" name="report_number" id="dr-report-number" required />
        </div>
        <div className="field"><label htmlFor="dr-report-date">{s.reportDate}</label><input className="input" type="date" name="report_date" id="dr-report-date" /></div>
      </div>

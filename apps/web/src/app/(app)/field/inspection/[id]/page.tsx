...
export default async function FieldInspection({ params }: { params: Promise<{ id: string }> }) {
  ...
  const strings: WorkspaceStrings = {
    ...
    recoveryReturnedTitle: tr("field.ws.recovery.returnedTitle", "This inspection was returned for correction", "أُعيدت هذه الزيارة للتصحيح"),
    recoveryReturnedBody: tr("field.ws.recovery.returnedBody", "A reviewer returned this inspection on the server while this device held an in-progress copy. Reload the latest to see the correction scope before continuing.", "أعاد المراجِع هذه الزيارة على الخادم بينما يحمل هذا الجهاز نسخة قيد التنفيذ. أعد تحميل الأحدث لعرض نطاق التصحيح قبل المتابعة."),

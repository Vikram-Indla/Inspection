...
export default function Workspace({ inspection, items, library, serverResponses, serverEvidence, serverForms, serverFindings, serverViolations, serverItemStates, serverContext, vioConfig, evidenceLimits, actionDueDays, strings, evidenceUrls, prev, panel, inspectionNo, locale, userId, cancellation, cancelReasons, journeySchemaAvailable }: {
  ...
  async function submit() {
    ...
    // session. The inspector reloads the latest via the recovery banner. This
    // runs before every other gate: a stale device must not even be evaluated.
  ...
  async function finalizeSubmit(ack: SignatureAck) {
    ...
    // Derive from the version this device actually loaded, not from a re-read of
    // the prop: both express the same number, but loadedVersion is the value the
    // stale-version guard compares against, so the two can never disagree.
  ...
  if (submitted) {
    ...
    const completionText = locale === "ar" ? {
      ...
      pending: "معلّق",
      reportsNote: "تظهر روابط التقارير فقط بعد أن يوفّرها الخادم.",
      feedback: "رمز التقييم واستلام التقرير",
      feedbackBody: "لا ينطبق",
      feedbackNote: "يجب أن يوفر الخادم رابطًا عامًا ورابط تقرير يطابق صلاحياتك.",
      ...
      pending: "Pending",
      reportsNote: "Report links appear once the server provides them.",
      feedback: "Feedback & report QR",
      feedbackBody: "N/A",
      feedbackNote: "The server must provide a public link and a report link that matches your access.",

...
import { createFieldSampleCollectionReport, type FieldSampleCollectionResult } from "./actions";
import styles from "../incident-reports/incident-reports.module.css";

export type FieldSampleCollectionStrings = {
  collectorName: string; laboratoryName: string; region: string; department: string; reason: string;
...
// "Sample Collection Report" (node 361:32119). Same signature-reuse pattern
// as field/summons-notices/SummonsNoticeForm.tsx.
export default function FieldSampleCollectionReportForm({ locale, strings: s, sigStrings, context, contextBadge }: {
  locale: string; strings: FieldSampleCollectionStrings; sigStrings: SignaturePadStrings;
  context?: { factoryId?: string; visitId?: string; inspectionId?: string };
  contextBadge?: string;
}) {
  const [state, action, pending] = useActionState<FieldSampleCollectionResult, FormData>(createFieldSampleCollectionReport, {});
  const [signing, setSigning] = useState(false);
  const [ack, setAck] = useState<(SignatureAck & { attendance: "present" | "absent" | "objected" }) | null>(null);

  return (
    <form id="new-sample-collection-report" action={action} className={styles.card}>
      ...
      <div className={styles.grid2}>
        <div className="field"><label htmlFor="scr-collector">{s.collectorName}</label><input className="input" name="collector_name" id="scr-collector" /></div>
        <div className="field"><label htmlFor="scr-lab">{s.laboratoryName}</label><input className="input" name="laboratory_name" id="scr-lab" /></div>
        <div className="field"><label htmlFor="scr-region">{s.region}</label><input className="input" name="region" id="scr-region" /></div>
        <div className="field"><label htmlFor="scr-department">{s.department}</label><input className="input" name="department" id="scr-department" /></div>
      </div>
      <div className="field" style={{ marginBlockStart: "var(--space-3)" }}>
        <label htmlFor="scr-reason">{s.reason}</label>
        <textarea className="input" name="reason" id="scr-reason" rows={3} />
      </div>

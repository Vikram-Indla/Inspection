...
export function RecordSignature({ strings: s }: { strings: { record: string; recording: string; recorded: string; kind: string; outcome: string } }) {
  ...
  return (
    <form action={action} className="panel" style={{ padding: "var(--space-6)", display: "flex", gap: "var(--space-3)", alignItems: "flex-end", flexWrap: "wrap" }}>
      <div className="sq-field"><label className="sq-field__label" htmlFor="committee-signature-kind">{s.kind}</label>
        <select className="sq-input" name="kind" id="committee-signature-kind"><option value="signature">signature</option><option value="refusal">refusal</option></select></div>
      <div className="sq-field"><label className="sq-field__label" htmlFor="committee-signature-outcome">{s.outcome}</label>
        <select className="sq-input" name="outcome" id="committee-signature-outcome"><option value="captured">captured</option><option value="refused">refused</option><option value="queued">queued</option><option value="failed">failed</option></select></div>

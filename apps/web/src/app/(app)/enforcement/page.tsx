...
export default async function EnforcementPage() {
  ...
  return (
    <Shell current="/enforcement" title={t("mvp3.enforcement.title", "Enforcement and correction lifecycle")}
      ...
      <div className="ax-grid" style={{ marginBlock: "var(--space-4)" }}><section className="panel" style={{ padding: "var(--space-6)" }}><p className="t-caption">{t("mvp3.enforcement.openCases", "Open cases")}</p><strong className="ax-display">{(cases ?? []).filter(x => !["resolved","closed"].includes(x.status)).length}</strong></section><section className="panel" style={{ padding: "var(--space-6)" }}><p className="t-caption">{t("mvp3.enforcement.violations", "RLS-visible violations")}</p><strong className="ax-display">{(violations ?? []).length}</strong></section><section className="panel" style={{ padding: "var(--space-6)" }}><p className="t-caption">{t("mvp3.enforcement.objections", "Objections")}</p><strong className="ax-display">{(objections ?? []).length}</strong></section></div>
      <section className="panel stack" style={{ padding: "var(--space-6)" }}><h3>{t("mvp3.enforcement.board", "Case board")}</h3><div className="ax-tablewrap"><table className="ax-table"><thead><tr><th scope="col">{t("mvp3.enforcement.case", "Case")}</th><th scope="col">{t("common.source", "Source")}</th><th scope="col">{t("common.status", "Lifecycle")}</th><th scope="col">{t("common.updated", "Updated")}</th></tr></thead><tbody>{(cases ?? []).map(row => <tr key={row.id}><th scope="row"><a href={`/cases?case=${row.id}`}>{row.case_type}<div className="t-caption"><bdi>{row.id}</bdi></div></a></th><td>{row.origin_type ?? "unlinked"}<div className="t-caption"><bdi>{row.origin_ref ?? "—"}</bdi></div></td><td><span className="badge">{row.status}</span></td><td>{new Date(row.updated_at).toLocaleString()}</td></tr>)}{!error && !(cases ?? []).length ? <tr><td colSpan={4}>{t("mvp3.enforcement.empty", "No RLS-visible enforcement cases.")}</td></tr> : null}</tbody></table></div></section>

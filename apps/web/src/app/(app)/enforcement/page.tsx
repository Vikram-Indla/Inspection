...
export default async function EnforcementPage() {
  ...
  return (
    <Shell current="/enforcement" title={t("mvp3.enforcement.title", "Enforcement and correction lifecycle")}
      ...
      <section className="ax-surface ax-stack" style={{ padding: "var(--ax-space-300)" }}><h3>{t("mvp3.enforcement.board", "Case board")}</h3><div className="ax-tablewrap"><table className="ax-table"><thead><tr><th scope="col">{t("mvp3.enforcement.case", "Case")}</th><th scope="col">{t("common.source", "Source")}</th><th scope="col">{t("common.status", "Lifecycle")}</th><th scope="col">{t("common.updated", "Updated")}</th></tr></thead><tbody>{(cases ?? []).map(row => <tr key={row.id}><th scope="row"><a href={`/cases?case=${row.id}`}>{row.case_type}<div className="ax-caption"><bdi>{row.id}</bdi></div></a></th><td>{row.origin_type ?? "unlinked"}<div className="ax-caption"><bdi>{row.origin_ref ?? "—"}</bdi></div></td><td><span className="ax-lozenge">{row.status}</span></td><td>{new Date(row.updated_at).toLocaleString()}</td></tr>)}{!error && !(cases ?? []).length ? <tr><td colSpan={4}>{t("mvp3.enforcement.empty", "No RLS-visible enforcement cases.")}</td></tr> : null}</tbody></table></div></section>
      <section className="ax-surface ax-stack" style={{ padding: "var(--ax-space-300)", marginBlockStart: "var(--ax-space-200)" }}><h3>{t("mvp3.enforcement.objectionQueue", "Correction and objection queue")}</h3>{(objections ?? []).map(row => <div className="ax-row" key={row.id} style={{ justifyContent: "space-between", flexWrap: "wrap" }}><span><strong>{row.grounds}</strong><small className="ax-caption"> · {new Date(row.created_at).toLocaleDateString()}</small></span><span className="ax-lozenge">{row.status}</span></div>)}{!(objections ?? []).length ? <p className="ax-caption">{t("mvp3.enforcement.noObjections", "No RLS-visible objections.")}</p> : null}</section>
    </Shell>
  );
}

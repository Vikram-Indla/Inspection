    ...
    <main className="sq-content" data-saqeel-screen="APQ-S01">
      <section className="panel" aria-live="polite" aria-busy="true">
        <div className="panel-header">
          <h1 className="panel-title">{t("approval.queue.loading", "Loading Approval Queue")}</h1>
          <span className="badge badge-pending">{t("common.loading", "Loading")}</span>
        </div>
        <div className="panel-body stack">
          <div className="skeleton" />
          <div className="skeleton" />
          <p>{t("approval.queue.loadingBody", "Reading requests, objects, decisions and immutable receipts within your governed scope.")}</p>
        </div>
      </section>
    </main>
  );

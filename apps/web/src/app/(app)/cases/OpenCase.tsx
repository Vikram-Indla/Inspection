...
export function OpenCase({ factoryId, strings: s }: { factoryId: string | null; strings: { type: string; open: string; opening: string; opened: string; noFactory: string } }) {
  ...
  return (
    <form action={action} className="panel" style={{ padding: "var(--space-6)", display: "flex", gap: "var(--space-3)", alignItems: "flex-end", flexWrap: "wrap" }}>
      ...
      <div className="sq-field"><label className="sq-field__label" htmlFor="open-case-type">{s.type}</label>
        <select className="sq-input" name="case_type" id="open-case-type"><option value="correction">correction</option><option value="reinspection">reinspection</option><option value="appeal">appeal</option></select></div>

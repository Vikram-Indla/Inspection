...
export default async function Immediate() {
  ...
  const strings: ImmediateStrings = {
    ...
    chipGroupLabel: t("plan.imm.chipGroupLabel", "Immediate dispatch protections"),
    chipSatisfied: t("plan.imm.chipSatisfied", "satisfied"),
    chipBlocking: t("plan.imm.chipBlocking", "blocking"),
    chipTruth: t("plan.imm.chipTruth", "informational"),
    ...
    chipReasonBlocked: t("plan.imm.chipReasonBlocked", "select an urgency reason"),
    chipIdentityBlocked: t("plan.imm.chipIdentityBlocked", "enter factory identity"),
    chipIdentityRegistered: t("plan.imm.chipIdentityRegistered", "registered factory selected"),
    chipIdentityTemporary: t("plan.imm.chipIdentityTemporary", "temporary entity — flagged"),
    chipLocationBlocked: t("plan.imm.chipLocationBlocked", "enter valid coordinates"),
    chipLocationOfficial: t("plan.imm.chipLocationOfficial", "official registry pin"),
    chipLocationManual: t("plan.imm.chipLocationManual", "manually confirmed pin"),
    chipPackageBlocked: t("plan.imm.chipPackageBlocked", "select a published package"),
    chipInspectorAuto: t("plan.imm.chipInspectorAuto", "auto-assign"),
    chipInspectorManual: t("plan.imm.chipInspectorManual", "manual pick"),
    chipInspectorBlocked: t("plan.imm.chipInspectorBlocked", "no inspector available"),
    chipWindowImmediate: tr("plan.imm.chipWindowImmediate", "starts immediately — no planner window", "تبدأ فورًا — بلا نافذة للمخطط"),
    chipWindowSet: t("plan.imm.chipWindowSet", "custom window set"),
    chipWindowBlocked: t("plan.imm.chipWindowBlocked", "both-or-neither, end after start"),
    chipAuditDetail: t("plan.imm.chipAuditDetail", "every step recorded, append-only"),
    chipNotifyDetail: t("plan.imm.chipNotifyDetail", "queued with provider status — delivery not claimed"),
  };
  return (
    <Shell current="/planning" title={t("plan.imm.title", "Immediate visit — urgent dispatch")}

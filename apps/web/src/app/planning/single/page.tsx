...
export default async function SinglePlanning({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  ...
  if (!isPlanner) {
    return (
      <Shell current="/planning" title={t("plan.single.title", "Single visit planning")}>
        <EmptyState icon={<IconBlocked size={28} />} title={tr("plan.single.unauthorized.title", "Authorized role required", "يلزم دور مصرح له")}
  ...
  return (
    <Shell current="/planning" title={t("plan.single.title", "Single visit planning")}
      ...
      <Wizard query={q} results={graded} registryUnavailable={registryUnavailable} packages={(pkgs ?? []) as never} inspectors={inspectors} strings={strings} virtualEligible={virtualEligible} locale={locale} />
    </Shell>
  );
}

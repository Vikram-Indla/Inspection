const count=(v:number)=>v.toLocaleString("en");
...
{key:"factory_approved_outcome_recency",title:"Factory approved-outcome recency",definition:"Factory count plus exact latest approved timestamp; no overdue band.",trace:"AN-AC-010",format:(v:number)=>`${count(v)} factories`},
...
{key:"factory_coverage",title:"Factory coverage",definition:"Distinct factories carrying at least one visit in the period and scope.",trace:"AN-AC-016",format:(v:number)=>`${count(v)} factories`},
{key:"inspector_coverage",title:"Inspector coverage",definition:"Distinct inspectors holding a visible assignment; never utilization.",trace:"AN-AC-017",format:(v:number)=>`${count(v)} inspectors`},
...
{key:"review_turnaround_hours",title:"Review turnaround",definition:"Mean hours from inspection submission to the latest review decision. Reviews missing either timestamp are excluded.",trace:"AN-AC-026",format:(v:number)=>`${v.toFixed(1)} h`},

...
type Loaded={kind:"ok"|"degraded";rows:AnalyticsRpcRow[];refreshedAt:string;stale:boolean;affectedSource?:string};
export async function loadAnalytics(sb:SupabaseClient,q:AnalyticsQuery):Promise<Loaded|{kind:"unauthorized"}|{kind:"error";message:string}>{
  ...
  return{kind:affected?"degraded":"ok",rows,refreshedAt,stale:rows.some(r=>r.source_status==="stale"),...(affected?{affectedSource:affected.unavailable_reason??affected.metric_key}:{})};
}

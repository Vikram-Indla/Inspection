...
import { neutralAuditError, type ExpectedAuditEvent, type ReplayEvent } from "@/lib/audit-replay";
...
export async function loadAudit(params: AuditParams): Promise<AuditView> {
  const caseRef = params.caseRef;
  const caseRefUuid = UUID.test(caseRef) ? caseRef : null;
  ...
  if (caseRef) {
    ...
    for (let page = 0; page < 20; page++) {
      ...
      const rows = rawRows as SemanticRow[];
    ...
    const result = await sb.from("audit_semantic_events")
      ...
      .order("occurred_at", { ascending: false }).order("id", { ascending: false }).limit(PAGE_SIZE);
    const rawRows: unknown = result.data ?? [];
    semanticRows = rawRows as SemanticRow[];
  ...
  const events = [...semanticEvents, ...genericEvents]
  ...
  const [entries, registryRes, constraints] = versionId
    ? await Promise.all([
  ...
  const registry = new Map((registryRes.data ?? []).map(row => [row.event_type, row]));
  const constraintRows = constraints.data ?? [];
  const expected: ExpectedAuditEvent[] = (entries.data ?? []).flatMap(entry => {
  ...
  const ontologyLoaded = Boolean(versionId) && !versionError && !entries.error && !registryRes.error && !constraints.error && expected.length === 36;
  ...
  return {
    authorized,
    partialScope,
    semanticUnavailable: Boolean(semanticError) || !ontologyLoaded,
    sourceError: Boolean(genericResult.error),

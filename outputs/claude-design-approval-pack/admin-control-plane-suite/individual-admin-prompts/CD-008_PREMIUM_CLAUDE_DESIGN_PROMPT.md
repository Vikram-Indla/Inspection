# CD-008 — Premium Claude Design Prompt
...
CLAUDE DESIGN SCREEN PROMPT — CD-008 / SCR-ADM-030

You are designing Package library for Saqeel MVP1. This is a controlled high-fidelity redesign of an already implemented product, not a greenfield concept.

MANDATORY REPOSITORY DISCOVERY
Read AGENTS.md; product-contract/00_START_HERE.md; CURRENT_STATE.md; GATE_STATUS.md; DECISIONS_ACCEPTED_2026-07-12_SAQEEL.yaml; design/claude-design-mvp1/00_START_HERE.md; CURRENT_UI_BASELINE.md; authority/CODE_ROUTE_RECONCILIATION.csv; product-contract/screens/screen_route_catalogue.csv; FABLE_UNDERSTANDING_TRACEABILITY.csv; FABLE_ACCEPTANCE_UNDERSTANDING.csv; and these screen-specific sources: admin/packages/page.tsx; package_versions; SCR-ADM-030. Inspect the existing implementation at apps/web/src/app/admin/packages. Record branch, commit and dirty-worktree state. Do not design from screenshots alone.

CONTRACT
Screen: SCR-ADM-030. Route/mode: /admin/packages. Persona: Form Admin; Compliance Admin. Journey: P00. Engines: ENG-02, ENG-12. User job: Find the right package version and know whether it is safe for use. Requirements: Filter the 493-row acceptance ledger and 478-row traceability by this screen and journey; preserve every matched row.

DESIGN THESIS
A versioned product library where coverage, dependencies and effective scope lead the hierarchy.
Why this way: Planners and inspectors depend on the published package version; a package name alone is insufficient.
Signature differentiator: Coverage fingerprint summarizes sections, items, evidence rules, activities and unresolved dependencies.
Outlier/hard case: Old inspections must remain pinned to old package versions while new visits use the new version.

INFLUENCE AND ORIGINALITY
Study references R01, R02, R11, R12 in the pack’s Reference Library. Adopt this principle: Fiori list/object pairing and Carbon expandable data rows. Explicitly reject: No card marketplace or static template gallery. Saqeel’s DEC-011 tokens, typography, dark/light themes, EN/AR behavior and component grammar remain the visual authority.

MANDATORY OBJECTS
Package/version; scope; status; effective date; dependency health; usage count; coverage fingerprint; clone/preview/validate/publish.
Mandatory states: No package; draft; submitted; published; superseded; unresolved item; active dependency.
Blind spots to resolve visibly: Version immutability and effective-date overlap; preview must match field runtime; package size/offline footprint.
Art direction: Treat each package as a deployable governed artifact with visible consumers.

DELIVERABLE
Produce code-ready high-fidelity design, not a wireframe. Fit the existing route, React components, server actions, Supabase contracts, RBAC/RLS, state transitions, audit, offline outbox, version immutability and provider truth. Provide: (1) primary populated design; (2) the critical outlier/failure state; (3) applicable dark/light and EN/AR/RTL evidence; (4) desktop or 1024×1366 iPad portrait/landscape as appropriate; (5) component disposition—PRESERVE/UPDATE/CREATE/REMOVE, where REMOVE requires explicit human approval; (6) interaction, state, audit and data annotations; (7) PNG names CD-008_SCR-ADM-030_primary.png and CD-008_SCR-ADM-030_outlier.png.

# Claude Code MCP Implementation Prompt — CD-005 · Regulation Library
...
- Clean dedicated worktree on `feat/cd-005-regulation-library`; never touch `main`. Record `git rev-parse HEAD`, `git branch --show-current`, `git status --porcelain` at start and end.
...
- **Read before write:** open every file in §2 via MCP, produce `SOURCE_DISCOVERY_LOG_CD-005.md`. If a file contradicts the truth below, preserve the safer behavior, mark the leg `HANDOFF_BLOCKED`, and STOP.
...
## 1 · Identity
- Screen SCR-ADM-010 · contract vs current route: /admin/regulations (direct)
- Persona per §2. Design source of truth: `CD-005 Regulation Library.dc.html` + its handoff pack.
...
## 5 · Return
`SOURCE_DISCOVERY_LOG`, updated `IMPLEMENTATION_MANIFEST_CD-005.yaml` (literal paths; before/after responsibility; disposition; tests; rollback), row-complete `WIRING_MAP_CD-005.csv`, diff summary, §4 evidence, and a status line: `VERTICAL_SLICE_PASS` only if every proven leg is wired/tested/evidenced and every unproven leg is `HANDOFF_BLOCKED` with an owner; else `HANDOFF_BLOCKED` with the exact finding, missing evidence, owner, next action. Never emit approved/build-complete/ready-to-ship or a numeric self-score.

## 2 · Read first
`apps/web/src/app/admin/regulations/{page.tsx,Controls.tsx,actions.ts}`; migrations 0001/0002; the `cd-005-r1/` pack. Persona: Compliance Admin. Ownership split: CD-005 owns discovery/list/create-draft; CD-006 owns detail/clause-edit/publish.

## 3 · Proven truth & scope
- Reads: `regulations(id, code, title, issuing_authority, status, created_at)`; `regulation_clauses(id, regulation_id, clause_ref, title, applicability, legal_source)`; `inspection_items.clause_id`.
- Actions: create draft regulation; add clause; direct `draft → published`.
- **Build:** a dense legal register — lifecycle filtering, provenance/issuing-authority scan, and the Impact Footprint Rail (regulation → clauses → items; downstream unproven legs shown "Unavailable", unknown≠zero) + create-draft entry separated from irreversible actions. Detail handoff to CD-006 shown as a link target.
- **Blocked:** owner, effective dates, version history, maker-checker, published immutability, archive/clone/compare, overlap/dependency validation, package/violation impact counts, structured server-action errors, route guard. Audit: `regulations` row only; clause changes not audited; writer-persona audit timeline blocked.

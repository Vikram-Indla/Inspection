# Claude Code MCP Implementation Prompt — CD-006 · Regulation Detail and Version
...
- Clean dedicated worktree on `feat/cd-006-regulation-detail-and-version`; never touch `main`. Record `git rev-parse HEAD`, `git branch --show-current`, `git status --porcelain` at start and end.
...
- **Read before write:** open every file in §2 via MCP, produce `SOURCE_DISCOVERY_LOG_CD-006.md`. If a file contradicts the truth below, preserve the safer behavior, mark the leg `HANDOFF_BLOCKED`, and STOP.
...
## 1 · Identity
- Screen SCR-ADM-011 · contract vs current route: /admin/regulations/:id (contract, NOT implemented) → /admin/regulations detail logical mode
- Persona per §2. Design source of truth: `CD-006 Regulation Detail and Version.dc.html` + corrected pack `cd-006-r2/` (route/runtime memo, data-truth ledger, S01–S09 state matrix, wiring map, manifest, acceptance checklist, handoff).
...
## 5 · Return
`SOURCE_DISCOVERY_LOG`, updated `IMPLEMENTATION_MANIFEST_CD-006.yaml` (literal paths; before/after responsibility; disposition; tests; rollback), row-complete `WIRING_MAP_CD-006.csv`, diff summary, §4 evidence, and a status line: `VERTICAL_SLICE_PASS` only if every proven leg is wired/tested/evidenced and every unproven leg is `HANDOFF_BLOCKED` with an owner; else `HANDOFF_BLOCKED` with the exact finding, missing evidence, owner, next action. Never emit approved/build-complete/ready-to-ship or a numeric self-score.

## 2 · Read first
`apps/web/src/app/admin/regulations/{page.tsx,Controls.tsx,actions.ts}`; `supabase/migrations/0001_foundation.sql`, `0002_rbac_audit.sql`; the `cd-006-r2/` pack. Persona: Compliance Admin + Reviewer (no invented Approver).

## 3 · Proven truth & scope
- Reads: `regulations(code,title,issuing_authority,status,created_at)` + nested `regulation_clauses(id,regulation_id,clause_ref,title,applicability,legal_source)` + nested `inspection_items(code)` via `clause_id`.
- Actions: `createRegulation` (draft); `addClause` (accepts optional `legal_source`); `publishRegulation` = **DIRECT `draft → published`, NO mapped-clause validation**.
- Audit: `regulations` row only; clause changes NOT audited.
- **Build:** a detail dossier (metadata + clause navigator + clause→item dependency rail, downstream unproven legs shown "Not evaluated", unknown≠zero) reachable as a logical mode of `/admin/regulations`.
- **Blocked (disabled/annotated targets, wired to nothing):** mapped-clause publish validation gate, submit/approve, maker-checker, published immutability lock, version compare/lineage/supersede, dependency engine, dedicated detail route, route guard, and any audit timeline for the writer persona (no `audit_events` read grant). The unmapped-clause case is a blocker **disclosure**, not a working button.

# Claude Code MCP Implementation Prompt — CD-009 · Package Designer
...
- Clean dedicated worktree on `feat/cd-009-package-designer`; never touch `main`. Record `git rev-parse HEAD`, `git branch --show-current`, `git status --porcelain` at start and end.
...
- **Read before write:** open every file in §2 via MCP, produce `SOURCE_DISCOVERY_LOG_CD-009.md`. If a file contradicts the truth below, preserve the safer behavior, mark the leg `HANDOFF_BLOCKED`, and STOP.
...
## 1 · Identity
- Screen SCR-ADM-031 · contract vs current route: /admin/packages/:id/designer (contract, NOT implemented) → /admin/packages designer logical mode
- Persona per §2. Design source of truth: `CD-009 Package Designer.dc.html` + corrected pack `cd-009-r2/` (route/runtime memo, data-truth ledger, S01–S09 state matrix, wiring map, manifest, acceptance checklist, handoff).
...
## 5 · Return
`SOURCE_DISCOVERY_LOG`, updated `IMPLEMENTATION_MANIFEST_CD-009.yaml` (literal paths; before/after responsibility; disposition; tests; rollback), row-complete `WIRING_MAP_CD-009.csv`, diff summary, §4 evidence, and a status line: `VERTICAL_SLICE_PASS` only if every proven leg is wired/tested/evidenced and every unproven leg is `HANDOFF_BLOCKED` with an owner; else `HANDOFF_BLOCKED` with the exact finding, missing evidence, owner, next action. Never emit approved/build-complete/ready-to-ship or a numeric self-score.

## 2 · Read first
`apps/web/src/app/admin/packages/{page.tsx,DraftEditor.tsx,PackagePreview.tsx,ImpactPanel.tsx,PublishControls.tsx,actions.ts}`; the `cd-009-r2/` pack. Name the field-runtime source if you claim preview equivalence.

## 3 · Proven truth & scope
- Working `DraftEditor` controls: edit section title, set section mandatory flag, add/remove item code, add section, `saveDraftDefinition` (writes only while status = `draft`). Array order is consumed by preview/runtime but there is **no reorder control**. `PackagePreview` = read-only projection (responses, evidence, conditional source text, guidance, clause, violation link, action-form shape) — **not a simulator**. Publish gate = `approveAndPublish` + distinct approver (CD-008).
- **Build:** governed three-pane studio (structure / field canvas / read-only preview) editing a draft version only; published versions open read-only (immutability).
- **Blocked — render as disabled `HANDOFF_BLOCKED` target panels, never active controls:** item reorder, condition authoring, per-item required/optional/conditional rules, scoring enable/disable, full evidence-model editor, action-form authoring, simulation engine, circular-condition detector, route guard. Never describe the preview as a simulator; never present test-before-deploy as a working gate.

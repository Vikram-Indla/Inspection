# Claude Code MCP Implementation Prompt — CD-004 · Admin Control Plane Home
...
- Clean dedicated worktree on `feat/cd-004-admin-control-plane-home`; never touch `main`. Record `git rev-parse HEAD`, `git branch --show-current`, `git status --porcelain` at start and end.
...
- **Read before write:** open every file in §2 via MCP, produce `SOURCE_DISCOVERY_LOG_CD-004.md`. If a file contradicts the truth below, preserve the safer behavior, mark the leg `HANDOFF_BLOCKED`, and STOP.
...
## 1 · Identity
- Screen SCR-ADM-001 · contract vs current route: /admin (direct)
- Persona per §2. Design source of truth: `CD-004 Admin Control Plane Home.dc.html` + its handoff pack.
...
## 5 · Return
`SOURCE_DISCOVERY_LOG`, updated `IMPLEMENTATION_MANIFEST_CD-004.yaml` (literal paths; before/after responsibility; disposition; tests; rollback), row-complete `WIRING_MAP_CD-004.csv`, diff summary, §4 evidence, and a status line: `VERTICAL_SLICE_PASS` only if every proven leg is wired/tested/evidenced and every unproven leg is `HANDOFF_BLOCKED` with an owner; else `HANDOFF_BLOCKED` with the exact finding, missing evidence, owner, next action. Never emit approved/build-complete/ready-to-ship or a numeric self-score.

## 2 · Read first
`apps/web/src/app/admin/page.tsx`; migrations 0001/0002; the `cd-004-r2b/` pack (route/runtime memo, data-truth ledger, state matrix, wiring map, manifest, acceptance checklist, handoff). Persona: Admin family (control-plane overview).

## 3 · Proven truth & scope
- `page.tsx` runs six parallel reads: `engine_settings(engine, version_label, updated_at)` + counts for regulations, checklist items, published packages, violations, audit events.
- Model each Supabase result's `error`/null **independently** — one failed source renders `unavailable`, never `0`, and never implies platform-wide failure.
- **Build:** overview-only control plane — the Configuration Evidence Spine linking each governed lifecycle to evidence at the point of navigation. No approve/publish/module-lifecycle controls on the home.
- **Blocked (annotated targets):** Admin-family route guard; any engine-health / approval-age / SLA / staleness / provider-status / runtime-consumption inference from counts or `updated_at`; any audit timeline for compliance/form admin. Counts are provenance, not verdicts.

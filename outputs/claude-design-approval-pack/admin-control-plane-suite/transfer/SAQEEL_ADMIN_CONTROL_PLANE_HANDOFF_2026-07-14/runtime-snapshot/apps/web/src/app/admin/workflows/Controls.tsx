...
export function ProposeDraftForm({ baseVersionId, baseLabel, strings: s }: { baseVersionId: string; baseLabel: string; strings: WfStrings }) {
  ...
  return (
    <form action={formAction} className="ax-row" style={{ gap: "var(--ax-space-150)", alignItems: "flex-end", flexWrap: "wrap" }}>
      ...
      <div className="ax-field"><label className="ax-field__label">{s.newVersionLabel}</label>
        <input className="ax-input ax-numeric" name="version_label" placeholder={`${baseLabel}-next`} required /></div>
...
export function DraftPayloadEditor({ versionId, payload, strings: s }: { versionId: string; payload: object; strings: WfStrings }) {
  ...
  return (
    <form action={formAction} style={{ display: "flex", flexDirection: "column", gap: "var(--ax-space-150)" }}>
      ...
      <div className="ax-field"><label className="ax-field__label">{s.payloadLabel}</label>
        <textarea className="ax-input ax-numeric" name="payload" rows={14} defaultValue={JSON.stringify(payload, null, 2)} spellCheck={false} /></div>
